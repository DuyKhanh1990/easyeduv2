...
export function useClassMutations(
  ...
  options?: { onConflicts?: (conflicts: any[]) => void }
) {
  ...
  /** Remove a single student from the waiting list */
  const removeWaitingStudentMutation = useMutation({
    mutationFn: async (studentClassId: string) => {
      await apiRequest("DELETE", `/api/classes/${classId}/waiting-students/${studentClassId}`);
    },
    onMutate: async (studentClassId) => {
      const key = [`/api/classes/${classId}/waiting-students`];
      await queryClient.cancelQueries({ queryKey: key });
      const previous = queryClient.getQueryData(key);
      queryClient.setQueryData(key, (old: any[]) =>
        old ? old.filter((s) => s.id !== studentClassId) : old
      );
      return { previous };
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/classes"], exact: true });
      queryClient.refetchQueries({ queryKey: [`/api/classes/${classId}/waiting-students`], exact: true });
      toast({ title: "Đã xoá", description: "Đã loại học viên khỏi danh sách chờ" });
    },
    onError: (err: any, _, context: any) => {
      if (context?.previous !== undefined) {
        queryClient.setQueryData([`/api/classes/${classId}/waiting-students`], context.previous);
      }
      toast({ title: "Lỗi", description: err.message || "Không thể xoá học viên", variant: "destructive" });
    },
  });
  ...
  const updateSessionMutation = useMutation({
    ...
    onSuccess: (data: any) => {
      ...
      if (data?.conflicts?.length > 0) options?.onConflicts?.(data.conflicts);
    },
    onError: (err: any) => {
      toast({
        title: "Lỗi",
  ...
  const makeupMutation = useMutation({
    ...
    onSuccess: (_data, variables: any) => {
      ...
      queryClient.invalidateQueries({ queryKey: [`/api/classes/${classId}/sessions`] });
      queryClient.invalidateQueries({ queryKey: ["/api/classes", classId] });
      // Invalidate the classes list so the new makeup class appears immediately
      queryClient.invalidateQueries({
        predicate: (query) => {
          const key = query.queryKey[0] as string;
          return typeof key === "string" && key.startsWith("/api/classes") && !key.includes(classId);
        },
      });
      invalidateStudentSessions();
      invalidateCalendarQueries();
