...
export function useAssignmentsTab(enabled: boolean) {
  ...
  const defaultTo = toDateStr(new Date(today.getFullYear(), today.getMonth() + 1, 0));
  const [dateFrom, setDateFrom] = useState(defaultFrom);
  const [dateTo, setDateTo] = useState(defaultTo);

  const monthStr = toMonthStr(year, month);
  // Always use date range (defaults to full current month)
  ...
  const goToToday = () => {
    ...
    setDateFrom(toDateStr(new Date(today.getFullYear(), today.getMonth(), 1)));
    setDateTo(toDateStr(new Date(today.getFullYear(), today.getMonth() + 1, 0)));
  };
