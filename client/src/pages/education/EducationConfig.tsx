...
function ClassroomsTab({ perm }: { perm?: ConfigTabPerm }) {
  ...
  const { data: locations = [] } = useQuery<Location[]>({ queryKey: ["/api/locations"], staleTime: STATIC_STALE_TIME });
...
function SubjectsTab({ perm }: { perm?: ConfigTabPerm }) {
  ...
  const { data: subjects = [], isLoading } = useQuery<any[]>({ queryKey: ["/api/subjects"], staleTime: STATIC_STALE_TIME });
...
function OnlineLearningTab() {
  const { toast } = useToast();
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [editingRule, setEditingRule] = useState<OnlineLearningRule | null>(null);

  const { data: locations = [] } = useQuery<Location[]>({ queryKey: ["/api/locations"], staleTime: STATIC_STALE_TIME });
  const { data: rules = [], isLoading: rulesLoading } = useQuery<OnlineLearningRule[]>({
    queryKey: ["/api/online-learning-rules"],
    queryFn: async () => {
      const res = await fetch("/api/online-learning-rules", { credentials: "include" });
  ...
  const createMutation = useMutation({
    mutationFn: async (data: Omit<OnlineLearningRule, "id">) => {
      const res = await apiRequest("POST", "/api/online-learning-rules", data);
      return res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/online-learning-rules"] });
      toast({ title: "Đã thêm cấu hình học online" });
      setIsDialogOpen(false);
      setEditingRule(null);
    },
    onError: (err: any) => toast({ title: err.message || "Lỗi", variant: "destructive" }),
  });

  const updateMutation = useMutation({
    mutationFn: async ({ id, ...data }: OnlineLearningRule) => {
      const res = await apiRequest("PUT", `/api/online-learning-rules/${id}`, data);
      return res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/online-learning-rules"] });
      toast({ title: "Đã cập nhật cấu hình học online" });
      setIsDialogOpen(false);
      setEditingRule(null);
    },
    onError: (err: any) => toast({ title: err.message || "Lỗi", variant: "destructive" }),
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const res = await apiRequest("DELETE", `/api/online-learning-rules/${id}`);
      return res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/online-learning-rules"] });
  ...
  const form = useForm<OnlineLearningForm>({
    defaultValues: {
      locationId: "",
      earlyEntryMinutes: "0",
      lateEntryMinutes: "0",
  ...
  const handleSubmit = (data: OnlineLearningForm) => {
    if (!data.locationId) {
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        <CardTitle className="text-base font-semibold flex items-center gap-2">
          <Monitor className="h-4 w-4" /> Học online
        </CardTitle>
        <Button
          size="sm"
          onClick={() => { setEditingRule(null); setIsDialogOpen(true); }}
          data-testid="btn-add-online-rule"
        >
      ...
      <CardContent>
        {rules.length === 0 ? (
          <p className="text-sm text-muted-foreground text-center py-8">Chưa có cấu hình nào. Nhấn "Thêm mới" để tạo.</p>
        ) : (
          <Table>
            <TableHeader>
              <TableRow>
            ...
            <TableBody>
              {rules.map((rule) => (
                <TableRow key={rule.id}>
                  <TableCell className="font-medium">{locations.find((l) => l.id === rule.locationId)?.name || "—"}</TableCell>
      ...
      <Dialog open={isDialogOpen} onOpenChange={(open) => { setIsDialogOpen(open); if (!open) setEditingRule(null); }}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{editingRule ? "Sửa cấu hình học online" : "Thêm cấu hình học online"}</DialogTitle>
          </DialogHeader>
          <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-5">
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium">Thời gian vào</label>
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  <label className="text-xs text-muted-foreground">Vào sớm trước</label>
                  <div className="flex items-center gap-1.5">
                    <Input type="number" min="0" {...form.register("earlyEntryMinutes")} className="w-full" />
                    <span className="text-sm text-muted-foreground shrink-0">phút</span>
                  </div>
                </div>
                <div className="space-y-1.5">
                  <label className="text-xs text-muted-foreground">Vào muộn sau</label>
                  <div className="flex items-center gap-1.5">
                    <Input type="number" min="0" {...form.register("lateEntryMinutes")} className="w-full" />
                    <span className="text-sm text-muted-foreground shrink-0">phút</span>
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-1.5">
                <label className="text-xs text-muted-foreground">Kết thúc sớm</label>
                <div className="flex items-center gap-1.5">
                  <Input type="number" min="0" {...form.register("earlyEndMinutes")} className="w-32" />
            ...
            <DialogFooter>
              <Button type="button" variant="outline" onClick={() => { setIsDialogOpen(false); setEditingRule(null); }}>Huỷ</Button>
              <Button type="submit">{editingRule ? "Lưu" : "Thêm"}</Button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
...
// ─── Giới hạn điểm danh tab ───────────────────────────────────────────────────
type Role = { id: string; name: string; description?: string | null };

type AttendanceLimitRule = {
  id: string;
...
function DurationInput({
  days, hours, minutes,
  onDaysChange, onHoursChange, onMinutesChange,
}: {
    ...
    <div className="flex items-center gap-3">
      <div className="flex items-center gap-1.5">
        <Input
          ...
          className="w-20 text-center" placeholder="0"
        />
...
function AttendanceLimitTab() {
  const { toast } = useToast();
  type DeptWithRoles = { id: string; name: string; roles: Role[] };
  const { data: departments = [], isFetched: deptsFetched } = useQuery<DeptWithRoles[]>({
    queryKey: ["/api/departments"],
    staleTime: STATIC_STALE_TIME,
  });

  // Lấy tất cả vai trò trừ phòng Khách hàng
  const roleOptions = departments
    .filter(d => !d.name.toLowerCase().includes("khách hàng"))
    .flatMap(d => d.roles.map(r => ({ label: r.name, value: r.id })));
  ...
  const {
    ...
    beforeDays: number; beforeHours: number; beforeMinutes: number;
    afterDays: number; afterHours: number; afterMinutes: number;
    roleIds: string[];
  }>({
    queryKey: ["/api/system-settings/attendance-limit"],
    retry: 3,
    retryDelay: 800,
  });

  // Chỉ mount MultiSelect khi departments ĐÃ load VÀ config đã hoàn thành (success hoặc hết retry)
  const showMultiSelect = (configSuccess || configError) && deptsFetched;

  useEffect(() => {
    if (!savedConfig) return;
    setBeforeDays(String(savedConfig.beforeDays));
    setBeforeHours(String(savedConfig.beforeHours));
    setBeforeMinutes(String(savedConfig.beforeMinutes));
    setAfterDays(String(savedConfig.afterDays));
    setAfterHours(String(savedConfig.afterHours));
    setAfterMinutes(String(savedConfig.afterMinutes));
    setSelectedRoleIds(savedConfig.roleIds ?? []);
  }, [savedConfig]);

  const saveMutation = useMutation({
    mutationFn: () => apiRequest("PUT", "/api/system-settings/attendance-limit", {
      beforeDays: parseInt(beforeDays) || 0,
      beforeHours: parseInt(beforeHours) || 0,
      beforeMinutes: parseInt(beforeMinutes) || 0,
      afterDays: parseInt(afterDays) || 0,
      afterHours: parseInt(afterHours) || 0,
      afterMinutes: parseInt(afterMinutes) || 0,
      roleIds: selectedRoleIds,
    }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/system-settings/attendance-limit"] });
      toast({ title: "Đã lưu cấu hình giới hạn điểm danh" });
    },
    onError: (err: any) => {
  ...
  const totalBeforeMinutes = (parseInt(beforeDays) || 0) * 1440
    + (parseInt(beforeHours) || 0) * 60
    + (parseInt(beforeMinutes) || 0);
  const totalAfterMinutes = (parseInt(afterDays) || 0) * 1440
    + (parseInt(afterHours) || 0) * 60
    + (parseInt(afterMinutes) || 0);

  return (
    <Card className="max-w-lg">
      <CardHeader className="pb-3">
        <CardTitle className="text-base font-semibold flex items-center gap-2">
          <Timer className="h-4 w-4" /> Giới hạn điểm danh
        </CardTitle>
        <p className="text-sm text-muted-foreground">
          Cấu hình khoảng thời gian cho phép điểm danh trước và sau giờ học theo từng vai trò.
        </p>
      </CardHeader>
      <CardContent className="space-y-6">
        {/* Điểm danh trước */}
        <div className="space-y-2">
          <Label className="text-sm font-medium">Điểm danh trước</Label>
          <p className="text-xs text-muted-foreground">
            Tính ngược từ giờ bắt đầu buổi học. VD: buổi học lúc 10h, nhập 2 giờ → có thể điểm danh từ 8h–10h.
          </p>
          <DurationInput
            days={beforeDays} hours={beforeHours} minutes={beforeMinutes}
            onDaysChange={setBeforeDays} onHoursChange={setBeforeHours} onMinutesChange={setBeforeMinutes}
          />
          {totalBeforeMinutes > 0 && (
            <p className="text-xs text-primary font-medium">
              → Cho phép điểm danh trước <span className="font-semibold">{totalBeforeMinutes} phút</span> ({beforeDays !== "0" && `${beforeDays} ngày `}{beforeHours !== "0" && `${beforeHours} giờ `}{beforeMinutes !== "0" && `${beforeMinutes} phút`}) so với giờ bắt đầu.
            </p>
          )}
        </div>
        ...
        {/* Điểm danh sau */}
        <div className="space-y-2">
          <Label className="text-sm font-medium">Điểm danh sau</Label>
          <p className="text-xs text-muted-foreground">
            Tính từ giờ bắt đầu buổi học về tương lai. VD: buổi học lúc 10h, nhập 1 giờ → có thể điểm danh từ 10h–11h.
          </p>
          <DurationInput
            days={afterDays} hours={afterHours} minutes={afterMinutes}
            onDaysChange={setAfterDays} onHoursChange={setAfterHours} onMinutesChange={setAfterMinutes}
          />
          {totalAfterMinutes > 0 && (
            <p className="text-xs text-primary font-medium">
              → Cho phép điểm danh sau <span className="font-semibold">{totalAfterMinutes} phút</span> ({afterDays !== "0" && `${afterDays} ngày `}{afterHours !== "0" && `${afterHours} giờ `}{afterMinutes !== "0" && `${afterMinutes} phút`}) kể từ giờ bắt đầu.
            </p>
          )}
        </div>
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            Để trống = áp dụng cho tất cả vai trò. Không bao gồm các vai trò thuộc phòng Khách hàng.
          </p>
          {showMultiSelect ? (
            <MultiSelect
              key={(savedConfig?.roleIds ?? []).join(",") || "empty"}
              options={roleOptions}
              onValueChange={setSelectedRoleIds}
              defaultValue={savedConfig?.roleIds ?? []}
...
export default function EducationConfig() {
  ...
  const visibleTabs = EDUCATION_TABS.filter(t => t.value === "history" || (isSubTabVisible(EDUCATION_CONFIG_HREF, t.value) && canViewTab(myPerms, t.value)));
  ...
  const { data: locations } = useQuery<Location[]>({ queryKey: ["/api/locations"], staleTime: STATIC_STALE_TIME });
  const { data: shifts, isLoading: isLoadingShifts } = useQuery<ShiftTemplate[]>({
  ...
  const createMutation = useMutation({
    mutationFn: async (data: InsertShiftTemplate) => (await apiRequest("POST", "/api/shift-templates", { ...data, type: "class" })).json(),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["/api/shift-templates", "class"] }); toast({ title: "Thành công", description: "Đã tạo ca học mới" }); setIsDialogOpen(false); },
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["/api/shift-templates", "class"] }); toast({ title: "Thành công", description: "Đã cập nhật ca học" }); setIsDialogOpen(false); setEditingShift(null); },
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["/api/shift-templates", "class"] }); toast({ title: "Thành công", description: "Đã xóa ca học" }); },
      ...
      <div className="space-y-6">
        ...
        <Tabs value={activeTab} onValueChange={handleTabChange}>
          ...
          {isSubTabVisible(EDUCATION_CONFIG_HREF, "score-sheets") && canViewTab(myPerms, "score-sheets") && (
           ...
           {isSubTabVisible(EDUCATION_CONFIG_HREF, "online-learning") && canViewTab(myPerms, "online-learning") && (
            <TabsContent value="online-learning" className="mt-4">
              <OnlineLearningTab />
            </TabsContent>
          )}

           <TabsContent value="history" className="mt-4 h-[calc(100vh-220px)]">
             <EducationConfigHistoryTab />
           </TabsContent>
        </Tabs>
      </div>
    </DashboardLayout>
