...
import { queryClient, apiRequest, STATIC_STALE_TIME, getAuthHeaders } from "@/lib/queryClient";
...
import {
  CalendarIcon, Search, X, ChevronDown, Clock, AlertCircle,
} from "lucide-react";
...
/* ─── Types ──────────────────────────────────────────────── */
interface SelectOption { id: string; label: string; sub?: string; color?: string; isActive?: boolean }

/* ─── UserMultiSelect ────────────────────────────────────── */
function UserMultiSelect({
  label, required, placeholder, options: optionsProp, selected, onChange, hint, isLoading, "data-testid": testId,
}: {
  ...
  const options: SelectOption[] = Array.isArray(optionsProp) ? optionsProp : [];
                ...
                return (
                  <div
                    ...
                    className={cn(
                      ...
                      isInactive ? "opacity-40 cursor-not-allowed" : "hover:bg-muted/50 cursor-pointer"
                    )}
                    onClick={() => !isInactive && toggle(opt.id)}
                  >
                    <Checkbox
                      checked={selected.includes(opt.id)}
                      disabled={isInactive}
                      onCheckedChange={() => !isInactive && toggle(opt.id)}
                    />
...
function SingleSelect({
  label, placeholder, options: optionsProp, value, onChange, "data-testid": testId,
}: {
...
function buildDefaultForm(defaultStatusId = "", defaultLevelId = ""): TaskFormData {
  ...
  return {
    ...
    content: "", attachments: [], subjectIds: [], managerIds: [],
...
export function CreateTaskDialog({
  open, onOpenChange, onCreated, initialTask, defaultSubjectIds, defaultLocationIds,
}: {
  open: boolean;
  ...
  /* ── Fetch all data — only when dialog is open ─── */
  const { data: locations = [] } = useQuery<any[]>({ queryKey: ["/api/locations"], staleTime: STATIC_STALE_TIME, enabled: open });
  const { data: departments = [] } = useQuery<any[]>({ queryKey: ["/api/departments"], staleTime: STATIC_STALE_TIME, enabled: open });
  const { data: taskStatuses = [] } = useQuery<TaskStatus[]>({ queryKey: ["/api/task-statuses"], staleTime: STATIC_STALE_TIME, enabled: open });
  const { data: taskLevels = [] } = useQuery<TaskLevel[]>({ queryKey: ["/api/task-levels"], staleTime: STATIC_STALE_TIME, enabled: open });
  ...
  const { data: staffRaw, isFetching: staffFetching } = useQuery<any>({
    ...
    queryFn: () => fetch(staffUrl, { credentials: "include", headers: getAuthHeaders() }).then(r => r.json()),
    enabled: open,
  });
  const { data: studentRaw, isFetching: studentFetching } = useQuery<any>({
    queryKey: ["/api/students", firstLocationId, "minimal"],
    queryFn: () => fetch(studentUrl, { credentials: "include", headers: getAuthHeaders() }).then(r => r.json()),
    enabled: open,
  });
  ...
  useEffect(() => {
    if (open) {
      if (initialTask) {
        const savedAtts: AttachedFile[] = Array.isArray(initialTask.attachments)
          ? (initialTask.attachments as any[])
              .filter((a: any) => a && a.name && a.url)
        ...
        setForm((prev) => ({
          ...
          subjectIds: defaultSubjectIds && defaultSubjectIds.length > 0 ? defaultSubjectIds : prev.subjectIds,
          locationIds: defaultLocationIds && defaultLocationIds.length > 0 ? defaultLocationIds : prev.locationIds,
        }));
      }
      setErrors({});
    }
  }, [open, initialTask, defaultStatus?.id, defaultLevel?.id, defaultSubjectIds, defaultLocationIds]);
  ...
  /* ── Option lists ─── */
  const locationOptions: SelectOption[] = (locations ?? []).map((l: any) => ({ id: l.id, label: l.name }));
  const departmentOptions: SelectOption[] = (departments ?? []).map((d: any) => ({ id: d.id, label: d.name }));
  ...
  const staffOptions: SelectOption[] = filteredStaff.map((s: any) => ({ id: s.id, label: s.code ? `${s.fullName} (${s.code})` : s.fullName, sub: s.email || s.phone || "", isActive: s.status !== "Không hoạt động" }));
  const studentOptions: SelectOption[] = filteredStudents.map((s: any) => ({ id: s.id, label: s.code ? `${s.code} - ${s.fullName}` : s.fullName, sub: s.type || "", isActive: s.accountStatus !== "Không hoạt động" }));
