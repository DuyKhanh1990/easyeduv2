...
export function StaffDialog({ open, onOpenChange, staff, allStaff = [] }: StaffDialogProps) {
  ...
  const codeAutoRef = useRef(!staff);
  ...
  // Show a preview using the selected role settings. The server regenerates
  // the value atomically when codeAutoGenerated remains true.
  useEffect(() => {
    if (!staff && selectedRoles.length > 0 && departments) {
      ...
      const abbreviation = firstRole.name
        ...
        .join("");
      const prefix = String(firstRole.codePrefix || `${abbreviation}-`).trim().replace(/-?$/, "-");
      const location = locations?.find((item: any) => item.id === selectedLocationIds[0]);
      const stem = firstRole.codeByLocationRole && location?.code
        ? `${prefix}${location.code.toUpperCase()}-`
        : prefix;

      const existingNumbers = allStaff
        .map(s => s.code)
        .filter(code => code && code.startsWith(prefix))
        .map(code => parseInt(code.substring(stem.length), 10))
        .filter(n => !isNaN(n));

      const nextNum = existingNumbers.length > 0 ? Math.max(...existingNumbers) + 1 : 1;
      const newCode = `${stem}${nextNum.toString().padStart(2, "0")}`;
      form.setValue("code", newCode);
      form.setValue("username", newCode.toLowerCase());
    }
  }, [selectedRoles, selectedLocationIds, departments, locations, staff, allStaff, form]);
  ...
  async function onSubmit(values: FormValues) {
    ...
    try {
      ...
      const payload = {
        ...
        codeAutoGenerated: !staff && codeAutoRef.current,
                    ...
                    <FormControl>
                      <Input
                        ...
                        onChange={(event) => {
                          ...
                          if (!staff) codeAutoRef.current = false;
                        }}
                        placeholder="VD: GV-01"
                        className={INP}
                      />
                    </FormControl>
                    <FormMessage />
