...
export function SplitScheduleDialog({
  ...
  const newAmount = Number(splitAmount) || 0;
  const carryOver = amount - newAmount;
  const isValid = newAmount > 0 && newAmount < amount;
      ...
      <DialogContent className="max-w-sm">
        ...
        <div className="space-y-4 py-2">
          ...
          <div className="space-y-1">
            <label className="text-sm font-medium">Số tiền giữ lại cho {label}</label>
            <Input
              type="number"
              value={splitAmount}
            ...
            />
            <p className="text-[11px] text-muted-foreground">Phần còn lại sẽ cộng vào đợt kế tiếp (hoặc tạo đợt mới)</p>
          </div>
          ...
          {newAmount > 0 && (
            <div className="rounded-lg border border-blue-200 bg-blue-50/50 p-3 space-y-2 text-sm">
              ...
              <div className="flex justify-between items-center">
                <span className="text-muted-foreground">{label} (giữ lại):</span>
                <span className={`font-medium ${isValid ? "text-blue-700" : "text-red-500"}`}>{newAmount.toLocaleString("vi-VN")} ₫</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-muted-foreground">Đợt kế tiếp (cộng thêm):</span>
                <span className={`font-medium ${carryOver > 0 ? "text-orange-600" : "text-red-500"}`}>{carryOver.toLocaleString("vi-VN")} ₫</span>
              </div>
              {!isValid && (
                <p className="text-xs text-red-500">Số tiền giữ lại phải lớn hơn 0 và nhỏ hơn {amount.toLocaleString("vi-VN")} ₫</p>
              )}
            </div>
          )}
        </div>
          ...
          <Button
            ...
            onClick={() =>
              splitMutation.mutate(
                { scheduleId, splitAmount: newAmount },
                {
                  onSuccess: () => {
