...
export function SingleChoiceDialog({ open, onClose, onSave, initialData }: SingleChoiceDialogProps) {
  ...
  async function handleAudioUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      toast({ title: "Tải audio lên thất bại", variant: "destructive" });
    } finally {
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex gap-2">
              ...
              >
                ...
                {uploadingAudio ? "Đang tải audio..." : "Tải audio lên"}
              </Button>
              <input ref={imageInputRef} type="file" accept="image/*" className="hidden" onChange={handleImageUpload} />
              <input ref={audioInputRef} type="file" accept="audio/*" className="hidden" onChange={handleAudioUpload} />
            </div>
            {form.imageUrl && (
              <img src={toMediaUrl(form.imageUrl)} alt="preview" className="mt-2 max-h-40 rounded-md border object-contain" />
            )}
            {form.audioUrl && (
              <audio src={toMediaUrl(form.audioUrl)} controls className="mt-2 w-full h-8" />
            )}
          </div>
