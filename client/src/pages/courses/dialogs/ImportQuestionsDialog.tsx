...
function extractAnswerKeyFromHtml(html: string): Map<number, string> {
  ...
  tables.forEach(table => {
    ...
    rows.forEach(row => {
      ...
      if (cells.length >= 2) {
        ...
        const match = leftText.match(/^Câu\s+(\d+)/i);
        if (match && rightText) {
          map.set(parseInt(match[1]), rightText.toUpperCase().trim());
        }
      }
    });
  });
...
function extractTextFromHtml(html: string): string {
  ...
  doc.body.querySelectorAll("p").forEach(p => {
    // Replace <br> with newline placeholder before getting textContent
    // so that soft-return lines (Shift+Enter in Word) are treated as separate lines
...
function parseWordText(text: string, questionType: string, answerKeyMap?: Map<number, string>): { questions: ParsedQuestion[]; errors: ParseError[] } {
  const questions: ParsedQuestion[] = [];
  const errors: ParseError[] = [];

  const blocks = text.split(/(?=^Câu\s+\d+\s*(?:[.:]|$))/im).map(b => b.trim()).filter(Boolean);
  ...
  if (questionType === "matching") {
    blocks.forEach((block, blockIdx) => {
      ...
      const inlineMatchM = firstLine.match(/^Câu\s+\d+\s*[.:]\s*(.+)/i);
      if (inlineMatchM) {
        questionText = inlineMatchM[1].trim();
      } else if (/^Câu\s+\d+\s*[.:]?\s*$/i.test(firstLine)) {
        for (let i = 1; i < lines.length; i++) {
          const l = lines[i];
          if (/^(\d+)\.\s+/i.test(l) || /^[A-Z]\.\s+/i.test(l)) { matchStartIdx = i; break; }
          if (l.trim()) { questionText = l.trim(); matchStartIdx = i + 1; break; }
        }
      } else {
      ...
      for (let i = matchStartIdx; i < lines.length; i++) {
        const line = lines[i];
  ...
  blocks.forEach((block, blockIdx) => {
    ...
    const firstLine = lines[0];
    let actualQuestionNum = questionNum;
    let content = "";
    let optStartIdx = 1;

    const inlineMatch = firstLine.match(/^Câu\s+(\d+)\s*[.:]\s*(.+)/i);
    if (inlineMatch) {
      actualQuestionNum = parseInt(inlineMatch[1]);
      content = inlineMatch[2].trim();
    } else if (/^Câu\s+(\d+)\s*[.:]?\s*$/i.test(firstLine)) {
      const headerNum = firstLine.match(/^Câu\s+(\d+)/i);
      if (headerNum) actualQuestionNum = parseInt(headerNum[1]);
      for (let i = 1; i < lines.length; i++) {
        const l = lines[i];
        if (/^[A-H]\.\s+/i.test(l) || /^(Đáp\s+án|Điểm|Giải\s+thích)\s*:/i.test(l)) {
          optStartIdx = i; break;
        }
        if (l.trim()) { content = l.trim(); optStartIdx = i + 1; break; }
      }
    } else {
    ...
    if (!correctAnswerRaw && answerKeyMap) {
      correctAnswerRaw = answerKeyMap.get(actualQuestionNum) ?? "";
    }
    if (!correctAnswerRaw) {
      errors.push({ row: questionNum, message: `Câu ${questionNum}: thiếu đáp án đúng (thêm dòng "Đáp án: A" hoặc dùng bảng đáp án)` });
      return;
    }
...
async function parseWordFile(file: File, questionType: string): Promise<{ questions: ParsedQuestion[]; errors: ParseError[] }> {
  ...
  const answerKeyMap = extractAnswerKeyFromHtml(htmlResult.value);
  const text = extractTextFromHtml(htmlResult.value);
  return parseWordText(text, questionType, answerKeyMap.size > 0 ? answerKeyMap : undefined);
}
...
function makeAnswerTable(rows: { q: string; a: string }[]) {
  const headerRow = new TableRow({
    children: [
      new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "Câu hỏi", bold: true })] })], width: { size: 50, type: WidthType.PERCENTAGE } }),
      new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: "Đáp án đúng", bold: true })] })], width: { size: 50, type: WidthType.PERCENTAGE } }),
    ],
  });
  const dataRows = rows.map(r => new TableRow({
    children: [
      new TableCell({ children: [new Paragraph({ children: [new TextRun(r.q)] })] }),
      new TableCell({ children: [new Paragraph({ children: [new TextRun(r.a)] })] }),
    ],
  }));
    ...
    sections: [
      {
        children: [
          ...
          new Paragraph({ children: [new TextRun("D. Huế")], spacing: { after: 60 } }),
          ...
          new Paragraph({ children: [new TextRun("D. gone")], spacing: { after: 60 } }),
          ...
          new Paragraph({ children: [new TextRun({ text: "Bảng đáp án (đặt đầu hoặc cuối file)", bold: true })], spacing: { before: 240, after: 120 } }),
          makeAnswerTable([
            { q: "Câu 1", a: "A" },
            { q: "Câu 2", a: "B" },
          ]),
    ...
    sections: [
      {
        children: [
          ...
          new Paragraph({ children: [new TextRun("D. Jakarta")], spacing: { after: 60 } }),
          new Paragraph({ children: [new TextRun("Điểm: 2")], spacing: { after: 60 } }),
          ...
          new Paragraph({ children: [new TextRun("D. Assembly")], spacing: { after: 60 } }),
          new Paragraph({ children: [new TextRun("Điểm: 1")], spacing: { after: 60 } }),
          new Paragraph({ children: [new TextRun("Giải thích: Java và Python là ngôn ngữ lập trình hướng đối tượng")], spacing: { after: 240 } }),

          new Paragraph({ children: [new TextRun({ text: "Bảng đáp án (đặt đầu hoặc cuối file — nhiều đáp án cách nhau bằng dấu phẩy)", bold: true })], spacing: { before: 240, after: 120 } }),
          makeAnswerTable([
            { q: "Câu 1", a: "A,B,D" },
            { q: "Câu 2", a: "A,C" },
          ]),
...
// ─── Download fixed/normalised Word file from parsed questions ────────────────

async function downloadFixedWordFile(questions: ParsedQuestion[], questionType: string) {
  ...
  questions.forEach((q, idx) => {
    ...
    if (questionType === "single_choice" || questionType === "multiple_choice") {
      children.push(new Paragraph({ children: [new TextRun({ text: `Câu ${num}: ${q.content}`, bold: true })], spacing: { after: 120 } }));
      (q.options as { id: string; text: string }[]).forEach(opt => {
        children.push(new Paragraph({ children: [new TextRun(`${opt.id}. ${opt.text}`)], spacing: { after: 60 } }));
      });
      children.push(new Paragraph({ children: [new TextRun(`Đáp án: ${q.correctAnswer}`)], spacing: { after: 60 } }));
      children.push(new Paragraph({ children: [new TextRun(`Điểm: ${q.score}`)], spacing: { after: 60 } }));
      if (q.explanation) {
        children.push(new Paragraph({ children: [new TextRun(`Giải thích: ${q.explanation}`)], spacing: { after: 240 } }));
      } else {
        children.push(new Paragraph({ children: [new TextRun("")], spacing: { after: 240 } }));
      }
    } else if (questionType === "fill_blank") {
      children.push(new Paragraph({ children: [new TextRun({ text: `Câu ${num}: ${q.content}`, bold: true })], spacing: { after: 100 } }));
      (q.options as { id: string; answers: string[]; score: number }[]).forEach(blank => {
        children.push(new Paragraph({ children: [new TextRun(`Ô ${blank.id}:`)], spacing: { after: 60 } }));
        children.push(new Paragraph({ children: [new TextRun(`Đáp án: ${blank.answers.join(" | ")}`)], spacing: { after: 60 } }));
        children.push(new Paragraph({ children: [new TextRun(`Điểm: ${blank.score}`)], spacing: { after: 60 } }));
      });
      if (q.explanation) {
        children.push(new Paragraph({ children: [new TextRun(`Giải thích: ${q.explanation}`)], spacing: { after: 240 } }));
      } else {
      ...
      children.push(new Paragraph({ children: [new TextRun({ text: `Câu ${num}: ${q.content}`, bold: true })], spacing: { after: 120 } }));
      (q.options as { id: string; left: { text: string; imageUrl: string }; right: { text: string; imageUrl: string } }[]).forEach((pair, k) => {
        const leftLabel = String(k + 1);
        const rightLabel = String.fromCharCode(65 + k);
        const leftText = [pair.left.text, pair.left.imageUrl].filter(Boolean).join(" | ");
        const rightText = [pair.right.text, pair.right.imageUrl].filter(Boolean).join(" | ");
        children.push(new Paragraph({ children: [new TextRun(`${leftLabel}. ${leftText}`)], spacing: { after: 60 } }));
        children.push(new Paragraph({ children: [new TextRun(`${rightLabel}. ${rightText}`)], spacing: { after: 60 } }));
      });
      children.push(new Paragraph({ children: [new TextRun("")], spacing: { after: 240 } }));
    } else if (questionType === "essay") {
      let minWords = 50, maxWords = 200;
      try { const ca = JSON.parse(q.correctAnswer); minWords = ca.minWords ?? 50; maxWords = ca.maxWords ?? 200; } catch {}
      children.push(new Paragraph({ children: [new TextRun({ text: `Câu ${num}: ${q.content}`, bold: true })], spacing: { after: 60 } }));
      children.push(new Paragraph({ children: [new TextRun(`Số từ tối thiểu: ${minWords}`)], spacing: { after: 60 } }));
      children.push(new Paragraph({ children: [new TextRun(`Số từ tối đa: ${maxWords}`)], spacing: { after: 60 } }));
      children.push(new Paragraph({ children: [new TextRun(`Điểm: ${q.score}`)], spacing: { after: 240 } }));
    }
  });

  // Append answer key table for choice questions
  if ((questionType === "single_choice" || questionType === "multiple_choice") && questions.length > 0) {
    children.push(new Paragraph({ children: [new TextRun({ text: "Bảng đáp án", bold: true })], spacing: { before: 240, after: 120 } }));
    children.push(makeAnswerTable(questions.map((q, i) => ({ q: `Câu ${i + 1}`, a: q.correctAnswer }))));
  }

  const typeLabels: Record<string, string> = {
    single_choice: "trac_nghiem",
    multiple_choice: "nhieu_lua_chon",
    fill_blank: "dien_cho_trong",
    matching: "noi",
    essay: "tu_luan",
  };
  ...
  const link = document.createElement("a");
  link.href = url;
  link.download = `cau_hoi_${typeLabels[questionType] ?? "import"}_chuan.docx`;
  link.click();
  URL.revokeObjectURL(url);
}
                      ...
                      <div className="flex items-center justify-between gap-2">
                        ...
                        {/\.(docx?)$/i.test(selectedFile?.name ?? "") && (
                          <Button
                            variant="outline"
                            size="sm"
                            className="h-6 px-2 gap-1 text-xs text-blue-700 border-blue-200 hover:bg-blue-50 shrink-0"
                            onClick={() => downloadFixedWordFile(parsedQuestions, questionType)}
                          >
                            <Download className="w-3 h-3" />
                            Tải file Word chuẩn hoá
                          </Button>
                        )}
                      </div>
                    )}
