...
import {
  ...
  Clock, CheckCircle2, AlertCircle, Send, Loader2, RefreshCw,
} from "lucide-react";
...
// multiple_choice stores correctAnswer as "B,C,D" (comma-separated) OR JSON array
...
function multipleChoiceIsCorrect(correctAnswer: string | null | undefined, given: string[]): boolean {
  const correct = parseMultipleChoiceAnswers(correctAnswer);
  const givenNorm = given.map(s => String(s).trim().toUpperCase());
  if (correct.length === 0 || correct.length !== givenNorm.length) return false;
  return correct.every(c => givenNorm.includes(c));
}

// fill_blank stores correctAnswer as "{1}: word1; {2}: word2" OR JSON array.
// q.options (FillBlankItem[]) is the most reliable source when available.
function parseFillBlankAnswers(raw: string | null | undefined, options?: unknown): string[] {
  // Prefer options array: [{ id, answers: string[], score }]
  if (Array.isArray(options) && options.length > 0 && options[0] && (options[0] as any).answers) {
    const blanks = options as { id: string; answers: string[]; score: number }[];
    const sorted = [...blanks].sort((a, b) => Number(a.id) - Number(b.id));
    return sorted.map(b => (b.answers?.[0] ?? "").trim());
  }
  if (!raw) return [];
  const trimmed = raw.trim();
  try {
  ...
  // Parse "{1}: answer1; {2}: answer2" format
  return trimmed.split(";").map(s => {
    const colonIdx = s.indexOf(": ");
    return colonIdx >= 0 ? s.substring(colonIdx + 2).trim() : s.trim();
  }).filter(Boolean);
}
// Per-blank scoring: each blank in q.options has its own score.
// Returns how many points were earned for this fill_blank question.
function calcFillBlankEarned(q: { score: unknown; correctAnswer?: string | null; options?: unknown }, a: unknown): number {
  ...
  if (Array.isArray(q.options) && q.options.length > 0 && (q.options[0] as any).answers !== undefined) {
    ...
    sorted.forEach((blank, i) => {
      ...
      if (!givenVal) return;
      if (blank.answers.some(ans => ans.trim().toLowerCase() === givenVal)) {
        earned += parseFloat(String(blank.score)) || 0;
      }
    });
    return earned;
  }
...
// Returns per-blank correctness array for visual highlighting.
function getFillBlankCorrectness(q: { correctAnswer?: string | null; options?: unknown }, a: unknown): boolean[] {
  const given = ((a as string) || "").split("|||");
  if (Array.isArray(q.options) && q.options.length > 0 && (q.options[0] as any).answers !== undefined) {
    const blanks = q.options as { id: string; answers: string[]; score: number }[];
    const sorted = [...blanks].sort((ba, bb) => Number(ba.id) - Number(bb.id));
    return sorted.map((blank, i) => {
      const givenVal = (given[i] || "").trim().toLowerCase();
      return blank.answers.some(ans => ans.trim().toLowerCase() === givenVal);
    });
  }
  const correct = parseFillBlankAnswers(q.correctAnswer, q.options);
  return correct.map((c, i) => c.trim().toLowerCase() === (given[i] || "").trim().toLowerCase());
}
// ──────────────────────────────────────────────────────────────────────────────
    ...
    <span className="leading-loose">
      {parts.map((part, i) => {
        if (/^\{\d+\}$/.test(part)) {
          ...
          const result = blankCorrectness != null ? blankCorrectness[blankIdx] : null;
          return (
            <input
              key={i}
              type="text"
              ...
              className={cn(
                "inline-block min-w-[80px] max-w-[180px] border-b-2 mx-1 px-2 py-0.5 text-sm rounded-sm focus:outline-none",
                result === null
                  ? "border-primary/60 bg-primary/5 focus:border-primary"
                  : result
...
function QuestionCard({ sq, globalIndex, answers, onAnswer, showResult }: QuestionCardProps) {
  ...
  function isCorrect(): boolean {
    ...
    if (q.type === "matching") {
      if (!Array.isArray(q.options) || (q.options as any[]).length === 0) return false;
      const matchAnswers = ((answer as Record<string, string>) || {});
      return (q.options as any[]).every((pair: any, oi: number) => {
        const sel = String(matchAnswers[pair.id] ?? "");
        return sel !== "" && sel === String(oi);
      });
    }
    ...
    if (q.type === "multiple_choice") {
      return multipleChoiceIsCorrect(q.correctAnswer, (answer as string[]) || []);
    }
    if (q.type === "fill_blank") {
      const correctness = getFillBlankCorrectness(q, answer);
      return correctness.length > 0 && correctness.every(Boolean);
    }
    return false;
  }

  const isEssay = q.type === "essay";
  const isPending = showResult && isEssay;
  const correct = showResult && !isEssay ? isCorrect() : null;
  const fillBlankCorrectness = showResult && q.type === "fill_blank"
    ? getFillBlankCorrectness(q, answer)
    : undefined;
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-start gap-2">
          <span className={cn(
            ...
            showResult
              ? isPending ? "bg-yellow-400 text-white"
      ...
      {(q.type === "multiple_choice") && Array.isArray(q.options) && (
        <div className="space-y-2 pl-9">
          ...
          {(q.options as any[]).map((opt: any) => {
            ...
            const correctAnswers = parseMultipleChoiceAnswers(q.correctAnswer);
            const isCorrectOpt = showResult && correctAnswers.includes(String(opt.id).trim().toUpperCase());
      ...
      {(q.type === "essay") && (
        <div className="pl-9">
          ...
          {showResult && (
            <div className="mt-2 flex items-start gap-2 px-3 py-2 rounded-lg bg-yellow-50 dark:bg-yellow-950/30 border border-yellow-200 dark:border-yellow-700">
      ...
      {(q.type === "matching") && Array.isArray(q.options) && (
        <div className="pl-9 space-y-2">
          ...
          {(q.options as any[]).map((pair: any, oi: number) => {
            ...
            if (showResult) {
              const selectedVal = String(matchAnswers[pair.id] ?? "");
              const isPairCorrect = selectedVal !== "" && selectedVal === String(oi);
              const selectedRight = selectedVal !== "" ? rightOptions[Number(selectedVal)] : null;
              const correctRight = rightOptions[oi];
              return (
                <div key={pair.id ?? oi} className="space-y-1">
                  <div className="flex items-center gap-3">
                    <div className="flex-1 px-3 py-2 rounded-lg border bg-muted/20 text-sm font-medium">
                      {pair.left?.text || "—"}
                    </div>
                    ...
                    <div className={cn(
                      ...
                      {selectedVal === "" ? "— Chưa chọn —" : (selectedRight?.text || "—")}
                    </div>
                    {isPairCorrect
                  ...
                  {!isPairCorrect && (
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 px-3 py-2 rounded-lg border border-green-400 bg-green-50 dark:bg-green-950/30 text-green-700 dark:text-green-300 text-sm flex items-center gap-1.5">
...
export function ExamTakingDialog({ exam, open, onClose, onSubmitSuccess, readonlySubmission, classId }: Props) {
  ...
  const [draftToRestore, setDraftToRestore] = useState<Answers | null>(null);
  ...
  // localStorage draft key — per exam per user
  function draftKey() {
    return `exam_draft_${exam.id}_${authUser?.id ?? "guest"}`;
  }

  // Core start logic (called after draft decision)
  function doStartExam(restoredAnswers?: Answers) {
    startTimeRef.current = Date.now();
    if (restoredAnswers) {
      setAnswers(restoredAnswers);
      answersRef.current = restoredAnswers;
    }
    // P2: notify server of session start (fire-and-forget)
    apiRequest("POST", `/api/exams/${exam.id}/start-session`).catch(() => {});
    setPhase("taking");
  ...
  // Check for saved draft before starting
  function startExam() {
    try {
      ...
      if (raw) {
        ...
        if (
          draft.examId === exam.id &&
          ageHours < 24 &&
          draft.answers &&
          Object.keys(draft.answers).length > 0
        ) {
          setDraftToRestore(draft.answers as Answers);
  ...
  function handleRestoreDraft() {
    setShowRestoreDialog(false);
    doStartExam(draftToRestore ?? undefined);
    setDraftToRestore(null);
  }
    ...
    try {
      ...
      allQs.forEach(sq => {
        ...
        if (q.type === "single_choice" && a === q.correctAnswer) totalEarned += scoreVal;
          ...
          if (multipleChoiceIsCorrect(q.correctAnswer, (a as string[]) || [])) totalEarned += scoreVal;
        } else if (q.type === "fill_blank") {
          totalEarned += calcFillBlankEarned(q, a);
        }
      });
      ...
      const partScores = currentSections.map((s, idx) => {
        ...
        s.questions.forEach(sq => {
          ...
          if (q.type === "single_choice") correct = a === q.correctAnswer;
            ...
            correct = multipleChoiceIsCorrect(q.correctAnswer, (a as string[]) || []);
          } else if (q.type === "fill_blank") {
      ...
      try { localStorage.removeItem(`exam_draft_${exam.id}_${authUser?.id ?? "guest"}`); } catch {}
      ...
      if (serverMsg.includes("đã hết")) {
        toast({ title: "Hết thời gian", description: serverMsg, variant: "destructive" });
      } else if (serverMsg.includes("chờ")) {
        toast({ title: "Nộp bài quá nhanh", description: serverMsg, variant: "destructive" });
      } else {
  ...
  }, [exam.id, authUser?.id, onSubmitSuccess, toast]);
  ...
  // Keep answersRef in sync with state (for stable closure access in effects)
  useEffect(() => {
    answersRef.current = answers;
  }, [answers]);

  // P1: Auto-save to localStorage every 30s while taking
  useEffect(() => {
    if (phase !== "taking") return;
    const key = `exam_draft_${exam.id}_${authUser?.id ?? "guest"}`;
  ...
  }, [phase, exam.id, authUser?.id]);

  // P1: Auto-save on answer change (debounced 2s)
  useEffect(() => {
    if (phase !== "taking") return;
    const key = `exam_draft_${exam.id}_${authUser?.id ?? "guest"}`;
    if (saveDebounceRef.current) clearTimeout(saveDebounceRef.current);
  ...
  }, [answers, phase, exam.id, authUser?.id]);

  // P1: Auto-save before page close/refresh
  useEffect(() => {
    if (phase !== "taking") return;
    const key = `exam_draft_${exam.id}_${authUser?.id ?? "guest"}`;
  ...
  function calcScore() {
    ...
    allQuestions.forEach(sq => {
      ...
      if (q.type === "single_choice") {
        ...
        if (multipleChoiceIsCorrect(q.correctAnswer, (a as string[]) || [])) earned += scoreVal;
      } else if (q.type === "fill_blank") {
        earned += calcFillBlankEarned(q, a);
      }
    });
    return { earned, total };
  }
    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-background" data-testid="exam-taking-fullscreen">
      ...
      {phase === "intro" && (
        <div className="flex-1 overflow-y-auto p-6 sm:p-8 flex flex-col items-center">
          {isLoading ? (
            <p className="text-muted-foreground text-sm mt-16">Đang tải bài kiểm tra...</p>
          ) : (
            <div className="max-w-lg w-full bg-card border rounded-2xl p-8 space-y-6 shadow-lg my-auto">
      ...
      {phase === "submitted" && (
        <div className="flex-1 flex overflow-hidden">
          {(() => {
            ...
            const canShowResult = !!readonlySubmission || !!(exam as any).showResult;
            ...
            allQuestions.forEach(sq => {
              const q = sq.question;
              const a = answers[sq.id];
              if (q.type === "essay") { essayCount++; return; }
              if (!a) return;
              let isCorrect = false;
              if (q.type === "single_choice") {
                isCorrect = a === q.correctAnswer;
              } else if (q.type === "multiple_choice") {
                isCorrect = multipleChoiceIsCorrect(q.correctAnswer, (a as string[]) || []);
              } else if (q.type === "fill_blank") {
                const correct = parseFillBlankAnswers(q.correctAnswer, q.options);
                const given = ((a as string) || "").split("|||");
                isCorrect = correct.every((c, i) => c.trim().toLowerCase() === (given[i] || "").trim().toLowerCase());
              } else if (q.type === "matching" && Array.isArray(q.options)) {
                const matchAnswers = (a as Record<string, string>) || {};
                isCorrect = (q.options as any[]).every((pair: any, oi: number) => {
                  const sel = String(matchAnswers[pair.id] ?? "");
                  return sel !== "" && sel === String(oi);
                });
              }
            ...
            function getAnswerKeyLabel(sq: SectionQuestionRow): string {
              const q = sq.question;
              if (!q.correctAnswer && q.type !== "essay" && q.type !== "matching") return "—";
              if (q.type === "single_choice") {
                return q.correctAnswer || "—";
              }
              if (q.type === "multiple_choice") {
                return parseMultipleChoiceAnswers(q.correctAnswer).join(", ") || "—";
              }
              if (q.type === "fill_blank") {
                return parseFillBlankAnswers(q.correctAnswer, q.options).join(", ") || "—";
              }
              if (q.type === "essay") {
                return "Tự luận";
              }
            ...
            function getAnswerKeyNode(sq: SectionQuestionRow, idx: number) {
              const q = sq.question;
              if (q.type === "matching" && Array.isArray(q.options)) {
                const matchAnswers = ((answers[sq.id] as Record<string, string>) || {});
                return (
                  <div key={sq.id} className="py-1 border-b border-border/40 last:border-0 space-y-1">
                    ...
                    {(q.options as any[]).map((pair: any, oi: number) => {
                      ...
                      const isPairCorrect = selectedVal !== "" && selectedVal === String(oi);
                      return (
                        <div key={pair.id ?? oi} className={cn(
                          "text-[10px] leading-tight px-1.5 py-0.5 rounded",
                          isPairCorrect ? "text-green-700 dark:text-green-300" : "text-foreground"
                        )}>
            ...
            return (
              <>
                ...
                <div className="w-[30%] border-r flex flex-col overflow-y-auto bg-muted/10 shrink-0">
                        ...
                        <div className="flex flex-col items-center gap-2 text-sm">
                          ...
                          {essayCount > 0 && (
                            <span className="flex items-center gap-1 text-xs text-yellow-600">
                              ...
                              <span>{essayCount} câu tự luận chờ giáo viên chấm</span>
                            </span>
                          )}
                        </div>
                      )}
                ...
                {canShowResult ? (
                  /* SHOW RESULT: Part tabs + questions + answer key sidebar */
                  <div className="flex-1 flex overflow-hidden">
                    {/* Middle: Part tabs + questions */}
                    <div className="flex-1 flex flex-col overflow-hidden">
                      {/* Part tab buttons */}
                    ...
                    {/* Right: Answer key sidebar */}
                    <div className="w-44 border-l flex flex-col bg-muted/5 shrink-0">
                      <div className="px-3 py-3 border-b bg-background">
                        <p className="text-xs font-bold text-muted-foreground uppercase tracking-wide">Đáp án</p>
                      </div>
                      <div className="flex-1 overflow-y-auto p-3 space-y-1">
                        {activeResultSection && activeResultSection.questions.map((sq, qi) => {
                          const matchingNode = getAnswerKeyNode(sq, activeResultStartIdx + qi);
                          if (matchingNode) return matchingNode;
                          const label = getAnswerKeyLabel(sq);
                          return (
                            <div key={sq.id} className="flex items-start gap-2 py-1 border-b border-border/40 last:border-0">
                              <span className="text-xs text-muted-foreground font-semibold shrink-0 w-5 pt-0.5">{activeResultStartIdx + qi + 1}</span>
                              <span className={cn(
                                "text-xs font-medium break-words leading-relaxed",
                                label === "Tự luận"
                  ...
                  /* NO RESULT: Only score summary, no question details */
                  <div className="flex-1 flex items-center justify-center bg-muted/5">
                    <div className="max-w-sm w-full mx-auto text-center space-y-4 px-6">
                      <div className="w-16 h-16 rounded-full bg-primary/10 flex items-center justify-center mx-auto">
                        <CheckCircle2 className="w-8 h-8 text-primary" />
                      </div>
                      <div>
                        <h3 className="text-lg font-bold mb-1">Đã nộp bài thành công</h3>
                        <p className="text-sm text-muted-foreground">Bài làm của bạn đã được ghi nhận.</p>
                      </div>
                      <div className={cn(
                        ...
                        <div className="text-4xl font-black text-primary mb-1">{earned.toFixed(1)}</div>
                        <div className="text-sm text-muted-foreground mb-3">/ {total.toFixed(1)} điểm</div>
                        {passed !== null && (
                          <div className={cn(
                            "inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full text-sm font-semibold",
                      ...
                      <p className="text-xs text-muted-foreground">Kết quả chi tiết không được hiển thị cho bài kiểm tra này.</p>
                    </div>
                  </div>
                )}
              </>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 p-4">
          <div className="bg-background rounded-2xl border shadow-xl p-6 max-w-sm w-full space-y-4">
            ...
            <div className="flex gap-3">
              <Button variant="outline" className="flex-1" onClick={() => setShowConfirm(false)} data-testid="btn-cancel-submit" disabled={isSubmitting}>
                Tiếp tục làm
              </Button>
              <Button
                className="flex-1"
              ...
              >
                {isSubmitting ? (
                  <><Loader2 className="w-4 h-4 mr-1.5 animate-spin" />Đang nộp...</>
                ) : (
                  <>Nộp bài</>
                )}
              </Button>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 p-4">
          <div className="bg-background rounded-2xl border shadow-xl p-6 max-w-sm w-full space-y-4">
            <div className="text-center space-y-2">
              ...
              <h3 className="text-base font-bold">Tiếp tục bài làm cũ?</h3>
              <p className="text-sm text-muted-foreground">
                Hệ thống phát hiện bài làm chưa hoàn thành từ lần trước.
                Bạn muốn tiếp tục hay bắt đầu lại?
              </p>
            </div>
            <div className="flex gap-3">
              <Button variant="outline" className="flex-1" onClick={handleStartFresh} data-testid="btn-start-fresh">
                Bắt đầu lại
              </Button>
              <Button className="flex-1" onClick={handleRestoreDraft} data-testid="btn-restore-draft">
                Tiếp tục
              </Button>
            </div>
          </div>
      ...
      {/* P0: Saving indicator overlay (submitted phase) */}
      {phase === "submitted" && isSubmitting && (
        <div className="fixed bottom-4 right-4 z-[70] flex items-center gap-2 bg-background border rounded-xl shadow-lg px-4 py-2.5 text-sm">
          <Loader2 className="w-4 h-4 animate-spin text-primary" />
          <span className="text-muted-foreground">Đang lưu bài làm...</span>
        </div>
      )}
    </div>
  );
