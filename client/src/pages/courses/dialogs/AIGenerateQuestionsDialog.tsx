...
export function AIGenerateQuestionsDialog({
  ...
  const { data: configuredProviders } = useQuery<{ openai: boolean; gemini: boolean; groq: boolean }>({
  ...
  useEffect(() => {
    ...
    const hasGroq = configuredProviders.groq;
    if (!hasOpenAI && !hasGemini && !hasGroq) return;
    if (hasOpenAI) { setProvider("openai"); return; }
    if (hasGemini) { setProvider("gemini"); return; }
    if (hasGroq) setProvider("groq");
