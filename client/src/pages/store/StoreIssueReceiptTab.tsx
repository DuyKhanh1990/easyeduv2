...
export function StoreIssueReceiptTab() {
  ...
  const [editData, setEditData] = useState<Partial<IssueReceiptFormData> & { id?: string } | undefined>(undefined);
  const [deleteItem, setDeleteItem] = useState<IssueReceiptRow | null>(null);
  const [invoiceLockedMsg, setInvoiceLockedMsg] = useState<string | null>(null);
  const [viewRow, setViewRow] = useState<IssueReceiptRow | null>(null);
  ...
  const { data: receipts = [], isLoading } = useQuery<IssueReceiptRow[]>({
    queryKey: ["/api/store/issue-receipts"],
    queryFn: () => apiRequest("GET", "/api/store/issue-receipts").then(r => r.json()),
  });
  ...
  const createMutation = useMutation({
    mutationFn: (body: any) => apiRequest("POST", "/api/store/issue-receipts", body).then(r => r.json()),
    onSuccess: () => { toast({ title: "Đã tạo phiếu xuất kho" }); invalidate(); setShowDialog(false); },
    onError: (e: any) => showError(e),
  });

  const updateMutation = useMutation({
    mutationFn: ({ id, body }: { id: string; body: any }) => apiRequest("PATCH", `/api/store/issue-receipts/${id}`, body).then(r => r.json()),
    onSuccess: () => { toast({ title: "Đã cập nhật phiếu xuất kho" }); invalidate(); setShowDialog(false); setEditId(null); },
    onError: (e: any) => showError(e),
  });

  const deleteMutation = useMutation({
    mutationFn: (id: string) => apiRequest("DELETE", `/api/store/issue-receipts/${id}`).then(r => r.json()),
    onSuccess: (data: any) => {
      if (data?.action === "cancelled") {
        toast({ title: "Đã hủy phiếu xuất kho", description: "Tồn kho đã được hoàn trả" });
      } else {
        toast({ title: "Đã xoá phiếu xuất kho" });
      }
      invalidate();
      setDeleteItem(null);
    },
  ...
  function sanitizeBody(data: IssueReceiptFormData, status: "draft" | "completed") {
    ...
    return {
      ...
      recipientId: toNullOrUUID((data as any).recipientId),
      note: data.note || null,
      discount: Number(data.discount) || 0,
      discountType: data.discountType,
      surcharge: Number(data.surcharge) || 0,
      surchargeType: data.surchargeType,
      promotionKeys: data.promotionKeys ?? [],
      surchargeKeys: data.surchargeKeys ?? [],
      manualPromotionRows: data.manualPromotionRows ?? [],
      manualSurchargeRows: data.manualSurchargeRows ?? [],
      ...
      items: data.items.map(({ _key, ...rest }) => ({
        ...
        salePrice: Number(rest.salePrice) || 0,
        stockBefore: Number(rest.stockBefore) || 0,
        productId: toNullOrUUID(rest.productId),
        unitId: toNullOrUUID(rest.unitId),
        promotionAmount: Number(rest.promotionAmount) || 0,
        surchargeAmount: Number(rest.surchargeAmount) || 0,
        lineTotal: Number(rest.lineTotal) || 0,
      })),
    };
  }

  function handleSave(data: IssueReceiptFormData, status: "draft" | "completed") {
  ...
  async function openEdit(row: IssueReceiptRow) {
    try {
      const detail = await apiRequest("GET", `/api/store/issue-receipts/${row.id}`).then(r => r.json());
      setEditData({
        id: detail.id,
        code: detail.code,
        name: detail.name,
        ...
        promotionKeys: detail.promotionKeys ?? [],
        surchargeKeys: detail.surchargeKeys ?? [],
        manualPromotionRows: detail.manualPromotionRows ?? [],
        manualSurchargeRows: detail.manualSurchargeRows ?? [],
        ...
        items: (detail.items ?? []).map((item: any, idx: number) => ({
          ...
          priceType: item.priceType ?? "money",
          starPrice: item.starPrice ?? 0,
          promotionAmount: parseFloat(item.promotionAmount ?? "0"),
          surchargeAmount: parseFloat(item.surchargeAmount ?? "0"),
  ...
  if (activeSubTab === "history") {
    return (
      <div className="flex flex-col h-full">
        {/* Sub-tab nav */}
        <div className="px-5 pt-3 pb-0 shrink-0 border-b border-border">
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-5 pt-3 pb-3 shrink-0 border-b border-border">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <Input
              placeholder="Tìm mã, tên, kho, người nhận..."
              value={search}
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <Button
              ...
              className="flex items-center gap-1.5 h-9 bg-orange-500 hover:bg-orange-600"
            >
              <Plus className="w-3.5 h-3.5" /> Thêm phiếu xuất kho
            </Button>
          </div>
        </div>
      </div>

      {/* Table - one fixed-height scroll container keeps both scrollbars visible */}
      <div className="flex-1 min-h-0 px-5 py-4">
      <div className="h-full min-h-0 border border-border rounded-xl overflow-x-scroll overflow-y-auto">
        <table className="text-xs border-separate border-spacing-0 w-full" style={{ minWidth: 1040 }}>
          <thead>
            <tr className="bg-muted">
              <th className="sticky top-0 z-20 bg-muted text-left px-4 py-2.5 font-semibold text-muted-foreground whitespace-nowrap">Ngày tạo</th>
              <th className="sticky top-0 z-20 bg-muted text-left px-4 py-2.5 font-semibold text-muted-foreground whitespace-nowrap">Cơ sở</th>
              <th className="sticky top-0 z-20 bg-muted text-left px-4 py-2.5 font-semibold text-muted-foreground whitespace-nowrap">Mã Phiếu</th>
              <th className="sticky top-0 z-20 bg-muted text-left px-4 py-2.5 font-semibold text-muted-foreground whitespace-nowrap">Tên phiếu</th>
              <th className="sticky top-0 z-20 bg-muted text-left px-4 py-2.5 font-semibold text-muted-foreground whitespace-nowrap">Kho xuất</th>
              <th className="sticky top-0 z-20 bg-muted text-left px-4 py-2.5 font-semibold text-muted-foreground whitespace-nowrap">Trạng thái</th>
              <th className="sticky top-0 z-20 bg-muted text-center px-4 py-2.5 font-semibold text-muted-foreground whitespace-nowrap">Số SP</th>
              <th className="sticky top-0 z-20 bg-muted text-center px-4 py-2.5 font-semibold text-muted-foreground whitespace-nowrap">SL</th>
              <th className="sticky top-0 z-20 bg-muted text-right px-4 py-2.5 font-semibold text-muted-foreground whitespace-nowrap">Tổng tiền</th>
              <th className="sticky top-0 z-20 bg-muted text-left px-4 py-2.5 font-semibold text-muted-foreground whitespace-nowrap">Người nhận</th>
              <th className="sticky top-0 z-20 bg-muted text-center px-4 py-2.5 font-semibold text-muted-foreground whitespace-nowrap">Hoá đơn</th>
              <th className="sticky top-0 z-20 bg-muted text-left px-4 py-2.5 font-semibold text-muted-foreground whitespace-nowrap">Người tạo</th>
              <th className="sticky top-0 right-0 z-30 bg-gray-50 text-center px-4 py-2.5 font-semibold text-muted-foreground whitespace-nowrap border-l border-border">Thao tác</th>
            </tr>
          </thead>
          <tbody>
            {isLoading ? (
              ...
              <tr>
                <td colSpan={13} className="text-center py-12 text-muted-foreground text-sm">
                  {hasActiveFilter
                    ...
                    : <>Chưa có phiếu xuất kho nào. Nhấn <span className="font-medium text-foreground">Thêm phiếu xuất kho</span> để bắt đầu.</>}
                </td>
              </tr>
              ...
              <tr key={r.id} className="border-t border-border hover:bg-muted/30 transition-colors">
                ...
                <td className="px-4 py-2.5 whitespace-nowrap">
                  <Badge
                    className={
                      r.status === "completed"
                        ? "bg-orange-100 text-orange-700 text-[10px] px-1.5 py-0.5 border-0"
                        : r.status === "cancelled"
                        ? "bg-red-100 text-red-600 text-[10px] px-1.5 py-0.5 border-0"
                        : "bg-amber-100 text-amber-700 text-[10px] px-1.5 py-0.5 border-0"
                    }
                  >
                    {r.status === "completed" ? "ĐÃ XUẤT" : r.status === "cancelled" ? "ĐÃ HỦY" : "NHÁP"}
                  </Badge>
                </td>
                ...
                <td className="px-4 py-2.5 whitespace-nowrap">{r.recipient_name ?? "—"}</td>
      ...
      <AlertDialog open={!!deleteItem} onOpenChange={o => !o && setDeleteItem(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {deleteItem?.status === "completed" ? "Hủy phiếu xuất kho?" : "Xoá phiếu xuất kho?"}
            </AlertDialogTitle>
                ...
                {deleteItem?.status === "completed" ? (
                  <>
                    ...
                    <ul className="list-disc list-inside space-y-0.5 text-xs">
                      <li>Tồn kho sẽ được hoàn trả về kho</li>
      ...
      {viewRow && (
        <StoreReceiptPrintDialog
          ...
          recipientName={viewRow.recipient_name}
