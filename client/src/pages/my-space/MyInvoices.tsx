...
import { FileText, Loader2, Receipt, CheckCircle2, Clock, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function MyInvoices() {
  ...
  const { totalPaid, totalUnpaid } = useMemo(() => {
    ...
    for (const inv of invoiceList) {
      ...
      if (inv.status === "paid") {
        paid += amt;
      } else if (inv.status === "unpaid" || inv.status === "partial" || inv.status === "debt") {
        unpaid += amt;
      }
    }
