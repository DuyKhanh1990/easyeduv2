...
export function CustomerForm({ initialData, onSubmit, isPending }: CustomerFormProps) {
  ...
  const codeAutoRef = useRef(!initialData);
  const usernameAutoRef = useRef(!initialData);
  ...
  useEffect(() => {
    ...
    codeAutoRef.current = !initialData;
    usernameAutoRef.current = !initialData;
    if (initialData) {
  ...
  const doSubmit = (data: FormData) => {
    const formattedData = {
      ...
      codeAutoGenerated: !initialData && codeAutoRef.current,
      usernameAutoGenerated: !initialData && usernameAutoRef.current,
    };
  ...
  const handleFormSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (dup.codeConflict && (initialData || !codeAutoRef.current)) {
      ...
      if (dup.usernameConflict && (initialData || !usernameAutoRef.current)) {
  ...
  useEffect(() => {
    if (code && !initialData && usernameAutoRef.current) {
            ...
            <div className="flex-1 space-y-4">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    className={INP}
                    placeholder={type === "Học viên" ? "HV-01" : "PH-01"}
                    {...form.register("code", {
                      onChange: () => {
                        if (!initialData) codeAutoRef.current = false;
                      },
                    })}
                  />
                ...
                <div className="space-y-1.5">
                  <label className={FL}>{t("customerForm.account")} <RequiredMark k="username" /></label>
                   <Input
                     ...
                     onChange={(e) => {
                       if (!initialData) usernameAutoRef.current = false;
