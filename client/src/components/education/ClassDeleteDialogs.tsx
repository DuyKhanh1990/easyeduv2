...
interface ClassDeleteDialogsProps {
  deleteTarget: { id: string; name: string; isActive?: boolean; hasAttendance?: boolean } | null;
...
function ActiveClassWarning({ isActive, hasAttendance }: { isActive?: boolean; hasAttendance?: boolean }) {
  if (!isActive && !hasAttendance) return null;

  const reasons: string[] = [];
  if (isActive) reasons.push("đang trong thời gian học");
  if (hasAttendance) reasons.push("đã có buổi điểm danh");

  return (
    <div className="mt-3 flex items-start gap-2 rounded-md border border-red-300 bg-red-50 dark:bg-red-950/30 dark:border-red-700 p-3 text-sm text-red-800 dark:text-red-300">
      <AlertTriangle className="h-4 w-4 mt-0.5 shrink-0 text-red-600 dark:text-red-400" />
      <span>
        <strong>Cảnh báo:</strong> Lớp này {reasons.join(" và ")}. Xóa lớp sẽ xóa toàn bộ dữ liệu điểm danh, lịch học và thông tin liên quan.
      </span>
    </div>
  );
}
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <ActiveClassWarning isActive={deleteTarget?.isActive} hasAttendance={deleteTarget?.hasAttendance} />
