...
import { VerticalSessionList } from "@/components/education/VerticalSessionList";
...
export function ScheduleTabContent({
  ...
  // when the parent sets open=true from external code (e.g. menu click).
  ...
  // Filter enrolled students locally (small set), but also apply search keyword
  ...
  const runWithConflictCheck = async (
    previewUrl: string,
    previewBody: any,
    execute: () => void,
  ) => {
    try {
      const res = await apiRequest("POST", previewUrl, previewBody);
      const data = await res.json();
      if (data.conflicts?.length > 0) {
        setPreviewConflicts(data.conflicts);
        setPendingAction(() => execute);
        return;
      }
    } catch {
      // pre-check failed — proceed with save
    }
    execute();
  };
  ...
  return (
    <>
      ...
      <UpdateSessionDialog
        ...
        sessionId={selectedClassSessionId || undefined}
        classData={classData}
        onConfirm={(data) =>
          runWithConflictCheck(
      ...
      <ChangeTeacherDialog
        ...
        onConfirm={(data) =>
          runWithConflictCheck(
            `/api/classes/${classId}/preview-teacher-conflicts`,
            { newTeacherIds: data.newTeacherIds, fromSessionId: data.fromSessionId, toSessionId: data.toSessionId },
                            ...
                            <tr key={studentId} className="border-b border-border hover:bg-muted/30 transition-colors">
                              ...
                              <td className="px-3 py-2">
                                ...
                                >
                                  ...
                                  <VoucherHint studentId={studentId} />
