                      ...
                      <TableCell>
                        <div className="flex items-center justify-center gap-1">
                          <Button
                            ...
                            data-testid={`button-view-grade-book-${book.id}`}
                ...
                <div className="h-full flex flex-col">
                  ...
                  <Table>
                    <TableHeader className="sticky top-0 bg-background z-10">
                      <TableRow>
                        <TableHead className="min-w-[180px] sticky left-0 bg-background z-20 border-r border-border/50">
                          Học viên
                        </TableHead>
                        {categories.map((cat: any) => {
                        ...
                        <TableHead className="w-[80px] text-center sticky right-0 bg-background z-20 border-l border-border/50 shadow-[-4px_0_6px_-2px_rgba(0,0,0,0.04)]">
                          Thao tác
                        </TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {displayedStudents.length === 0 ? (
                        ...
                        displayedStudents.slice(gradePage * gradePageSize, (gradePage + 1) * gradePageSize).map((student: any, idx: number) => {
                          const studentId = student.id || student.studentId;
                          const actualStudentId = student.studentId || student.student?.id;
                          const name =
                            student.fullName || student.full_name || student.student?.fullName || `Học viên ${gradePage * gradePageSize + idx + 1}`;
                  ...
                  {displayedStudents.length > 0 && (
                    <div className="flex items-center justify-between px-4 py-2 border-t border-border/40 shrink-0">
                      <span className="text-xs text-muted-foreground">
                        {gradePage * gradePageSize + 1}–{Math.min((gradePage + 1) * gradePageSize, displayedStudents.length)} / {displayedStudents.length} học viên
                      </span>
                      <div className="flex items-center gap-1.5">
                        <Select
                          value={String(gradePageSize)}
                          onValueChange={(v) => { setGradePageSize(Number(v)); setGradePage(0); }}
                        >
                        ...
                        <Button
                          ...
                          onClick={() => setGradePage(p => Math.max(0, p - 1))}
                          disabled={gradePage === 0}
                        >
                          <ChevronLeft className="h-3.5 w-3.5" />
                        </Button>
                        <span className="text-xs text-muted-foreground w-16 text-center">
                          {gradePage + 1}/{Math.max(1, Math.ceil(displayedStudents.length / gradePageSize))}
                        </span>
                        <Button
                          variant="outline"
                          size="icon"
                          className="h-7 w-7 border-border/50"
                          onClick={() => setGradePage(p => Math.min(Math.ceil(displayedStudents.length / gradePageSize) - 1, p + 1))}
                          disabled={gradePage >= Math.ceil(displayedStudents.length / gradePageSize) - 1}
                        >
      ...
      {viewingBook && (
        <GradeBookViewDialog
          ...
          book={{
            ...
            score_sheet_id: viewingBook.score_sheet_id,
            score_sheet_name: viewingBook.score_sheet_name,
            session_id: viewingBook.session_id,
            published: viewingBook.published,
          }}
        />
      )}
    </>
