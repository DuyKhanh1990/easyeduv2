...
export function BulkChangeCycleDialog({
  ...
  const mutation = useMutation({
    ...
    onSuccess: async (res: any) => {
      const data: any[] = res instanceof Response ? await res.json() : res;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-xl">
        <DialogHeader>
          ...
          {classSessionIndex !== undefined && (
            <p className="text-sm text-muted-foreground pt-1">
              Áp dụng từ <span className="font-semibold text-foreground">buổi thứ {classSessionIndex}</span> trở đi.
            </p>
          )}
        </DialogHeader>
