...
export function StudentFeeWalletTab({ studentId, open }: Props) {
  ...
  const from = transactions.length === 0 ? 0 : (page - 1) * pageSize + 1;
  const to = Math.min(page * pageSize, transactions.length);
          ...
          {!isLoading && !isError && transactions.length > 0 && (
            <>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="sticky top-0 z-10">
                    <tr className="bg-white border-b-2 border-blue-100 shadow-sm text-[10px] font-semibold text-gray-500 uppercase tracking-wider">
              ...
              <div className="px-5 py-3 border-t border-border flex items-center justify-between">
                <span className="text-xs text-muted-foreground">{from}–{to} / {transactions.length} giao dịch</span>
                <div className="flex items-center gap-2">
                  <span className="text-xs text-muted-foreground">Hiển thị:</span>
                  <select
                    value={pageSize}
                    onChange={e => { setPageSize(Number(e.target.value)); setPage(1); }}
                    className="text-xs border border-border rounded px-1.5 py-1 bg-background focus:outline-none"
                  >
                  ...
                  <button
                    ...
                    disabled={page === 1}
                    className="px-2 py-1 text-xs border border-border rounded bg-background hover:bg-muted disabled:opacity-40 disabled:cursor-not-allowed"
                  >‹</button>
