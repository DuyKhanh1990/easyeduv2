...
import {
  ...
  FileText, Download, Paperclip, CornerUpLeft, Pencil, Info,
} from "lucide-react";
...
function ChatWindow({
  ...
  const [members, setMembers] = useState<{ userId: string; displayName: string; role: string }[]>([]);
  ...
  // Fetch thành viên khi mở panel hoặc khi đổi topic
  useEffect(() => {
    if (!showMembers) return;
    setLoadingMembers(true);
    const token = getAuthToken();
    fetch(`/api/chat/topics/${encodeURIComponent(topicId)}/members`, {
      credentials: "include",
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-0.5">
          <button
            ...
            data-testid={`chat-members-${topicId}`}
            title="Thành viên"
          >
      ...
      {/* Members panel */}
      {showMembers && (
        <div className="flex-1 overflow-y-auto bg-white dark:bg-background flex flex-col">
          ...
          {loadingMembers ? (
            ...
            <div className="flex flex-col items-center justify-center flex-1 gap-2 text-muted-foreground py-8">
              ...
              <p className="text-xs">Không có thành viên</p>
            </div>
          ) : (
            <div className="px-2 py-2 flex flex-col gap-0.5">
              {members.map(m => (
                <div key={m.userId} className="flex items-center gap-2.5 px-2 py-2 rounded-lg hover:bg-muted/50 transition-colors">
                  <UserAvatar name={m.displayName} uid={m.userId} size="sm" />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium truncate leading-tight">{m.displayName}</p>
                    <p className="text-[10px] text-muted-foreground leading-none mt-0.5">
                      {m.role === "staff" ? "Giáo viên / Nhân viên" : "Học viên"}
                    </p>
                  </div>
                </div>
              ))}
      ...
      {!showMembers && <>
          ...
          <div className="px-3 py-2.5 border-t border-border/30 bg-white dark:bg-background shrink-0">
            ...
            {pendingFile && (
              <div className="mb-2 flex items-center gap-2 px-2.5 py-1.5 rounded-md border border-gray-200 bg-gray-50">
                {pendingFilePreview
                  ? <img src={pendingFilePreview} alt="" className="w-8 h-8 rounded object-cover shrink-0" />
                ...
                <button
                  ...
                  className="shrink-0 w-5 h-5 rounded-full hover:bg-gray-200 flex items-center justify-center text-gray-400 hover:text-gray-600"
                >
                  <X className="h-3 w-3" />
                </button>
              </div>
            ...
            <div className="flex items-center gap-2 bg-[#f0f2f5] dark:bg-muted rounded-full px-3 py-2 focus-within:ring-2 focus-within:ring-[#1877f2]/30 transition-all">
              ...
              <input
                ...
                onKeyDown={handleKey}
                placeholder={pendingFile ? "Thêm chú thích… (tuỳ chọn)" : (editingMsg ? "Sửa tin nhắn…" : (replyingTo ? "Trả lời…" : "Aa"))}
              ...
              <button
                ...
                className={cn(
                  ...
                  (input.trim() || pendingFile) && !isUploading ? "text-[#1877f2] hover:opacity-80 active:scale-90" : "text-muted-foreground/30"
                )}
              >
...
function ConversationList({
  ...
  const { toast } = useToast();
  const [view, setView] = useState<"list" | "create-group" | "new-message">("list");
  ...
  const handleDmSearchChange = useCallback((val: string) => {
    setDmSearch(val);
  ...
  // Reset DM search khi chuyển view
  useEffect(() => {
    if (view !== "new-message") {
      setDmSearch("");
      setUserResults([]);
  ...
  async function handleOpenP2P(user: UserSearchResult) {
    ...
    try {
      ...
      const data = await res.json();
      if (data.topicId) {
        onOpenP2P(data.topicId, user.displayName);
        onClose();
      } else {
  ...
  // All topics flat, sorted by latest message
  ...
  const totalTopicUnread = topics.reduce((s, t) => s + (t.unread ?? 0), 0);
  ...
  return (
    ...
    >
      ...
      <div className="shrink-0 flex items-center justify-between px-4 pt-4 pb-2">
        {view === "create-group" ? (
          ...
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-base font-bold text-foreground">Nhắn tin mới</h3>
          </div>
        ) : (
      ...
      {/* Search input — in new-message view */}
      {view === "new-message" && (
        <div className="shrink-0 px-4 pb-2">
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-muted-foreground" />
            <input
              ...
              placeholder="Tìm tên học viên, nhân viên…"
              autoFocus
              className="w-full pl-9 pr-3 py-2 text-sm bg-[#f0f2f5] dark:bg-muted rounded-full outline-none focus:ring-2 focus:ring-primary/20 transition-all placeholder:text-muted-foreground/60"
            />
          </div>
      ...
      {authed && view === "list" && (
        <div className="flex-1 min-h-0 overflow-y-auto">
          ...
          {!isStudent && (
            <div className="flex gap-2 px-4 py-2 border-b border-border/20">
              <button
                onClick={() => setView("create-group")}
                data-testid="create-group-button"
                className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl bg-[#e7f0ff] hover:bg-[#d0e4ff] text-[#1877f2] text-xs font-semibold transition-colors"
              >
                <Users className="h-3.5 w-3.5" />
                Tạo nhóm
              </button>
      ...
      {/* ── New message view (user search) ── */}
      {authed && view === "new-message" && (
        <div className="flex-1 min-h-0 overflow-y-auto">
          {!dmSearch.trim() ? (
...
export function ChatButton() {
  ...
  function openP2P(topicId: string, displayName: string) {
    if (displayName) {
      ...
      registerName(topicId, displayName);
    }
    openChat(topicId);
  }
