...
const mobileUpload = multer({
  storage: multer.diskStorage({
    ...
    filename: (_req, file, cb) => {
      ...
      cb(null, `task_${Date.now()}_${safeName}`);
    },
  }),
...
export function registerMobileTaskRoutes(app: Express) {
   ...
   * Returns statuses, levels, permissions, staff, students, locations, departments.
   * PHẢI đặt trước /:id để tránh Express match "meta" như task ID.
   */
  app.get("/api/mobile/tasks/meta", async (req, res) => {
    try {
      const perms = await resolveTaskPermissions(req);
      const [allStatuses, allLevels, allStaff, allStudents, allLocations, allDepartments] = await Promise.all([
        db.select().from(taskStatuses).orderBy(taskStatuses.position),
        db.select().from(taskLevels).orderBy(taskLevels.position),
        db.select({
          id: staff.id,
          ...
          code: staff.code,
        }).from(staff).orderBy(staff.fullName),
        db.select({
          id: students.id,
          fullName: students.fullName,
          code: students.code,
        }).from(students).orderBy(students.fullName),
        db.select({
          id: locations.id,
          name: locations.name,
      ...
      return res.json({
        ...
        statuses: allStatuses,
        levels: allLevels,
        // Dùng để populate dropdown Quản lý / Người thực hiện
        staff: allStaff.map(s => ({
          id: s.id,
          fullName: s.fullName || s.code || "",
          code: s.code,
        })),
        // Dùng để populate dropdown Đối tượng
      ...
      try {
        ...
        try {
          uploaded = await Promise.all(
            files.map(async (f) => {
              const url = await uploadFileToS3FromDisk(f.path, f.size, f.originalname, f.mimetype);
          ...
          for (const f of files) {
            fs.unlink(f.path, (e) => { if (e) console.warn("[MobileTasks] cleanup tmp failed:", e.message); });
          }
        }
  ...
  app.delete("/api/mobile/tasks/:id/attachments", async (req, res) => {
    try {
      ...
      // Trừ dung lượng file vừa xóa
      const { subtractFileByUrl } = await import("../lib/storage-usage");
      subtractFileByUrl(parsed.data.url).catch(() => {});
