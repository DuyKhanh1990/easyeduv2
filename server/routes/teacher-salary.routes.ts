...
import {
  ...
  ensureTeacherSalarySessionPackagesTable,
  getTeacherSalarySessionPackages,
  saveTeacherSalarySessionPackage,
  deleteTeacherSalarySessionPackage,
...
export async function registerTeacherSalaryRoutes(app: Express): Promise<void> {
  ...
  await ensureTeacherSalarySessionPackagesTable();
  ...
  app.get("/api/teacher-salary-tables/:id/session-packages", async (req, res) => {
    try {
      const packages = await getTeacherSalarySessionPackages(req.params.id);
      res.json(packages);
    } catch (error: any) {
      res.status(500).json({ message: error.message });
    }
  ...
  app.post("/api/teacher-salary-tables/:id/session-packages", async (req, res) => {
    try {
      const { sessionId, teacherId, packageId } = req.body;
      if (!sessionId || !teacherId || !packageId) {
        return res.status(400).json({ message: "Thiếu dữ liệu" });
      }
      await saveTeacherSalarySessionPackage(req.params.id, sessionId, teacherId, packageId);
      res.json({ success: true });
    } catch (error: any) {
      res.status(500).json({ message: error.message });
    }
  ...
  app.delete("/api/teacher-salary-tables/:id/session-packages/:sessionId/:teacherId", async (req, res) => {
    try {
      await deleteTeacherSalarySessionPackage(req.params.id, req.params.sessionId, req.params.teacherId);
      res.json({ success: true });
    } catch (error: any) {
      res.status(500).json({ message: error.message });
    }
