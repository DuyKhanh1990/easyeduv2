...
import { staffSalaryConfigs, insertStaffSalaryConfigSchema, courses, teacherSalaryPackages, staff, departments, staffAssignments, users } from "@shared/schema";
import { z } from "zod";
...
export function registerStaffSalaryConfigRoutes(app: Express): void {
  ...
  app.get("/api/staff/training-department", async (req, res) => {
    try {
      ...
      // Exclude the super admin account (username="admin")
      ...
      const staffList = await db
        ...
        .where(
          adminUserId
            ? (await import("drizzle-orm")).and(inArray(staff.id, staffIds), (await import("drizzle-orm")).sql`${staff.userId} != ${adminUserId}`)
