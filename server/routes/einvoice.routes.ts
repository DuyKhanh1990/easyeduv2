...
import { financePromotions, invoicePaymentSchedule } from "@shared/schema";
...
const signSchedulesSchema = z.object({
...
const configSchema = z.object({
  ...
  signingType: z.enum(["usb", "central"]).default("usb"),
});
...
export function registerEInvoiceRoutes(app: Express) {
  ...
  app.get("/api/einvoice/config", async (_req, res) => {
    try {
      ...
      res.json({
        ...
        signingType: cfg.signingType || "usb",
      });
    } catch (err: any) {
  ...
  // GET /api/einvoice/config/password — Trả password thật (chỉ admin)
  app.get("/api/einvoice/config/password", async (_req, res) => {
    try {
      const cfg = await matbao.getConfig();
      res.json({ password: cfg.password || "" });
    } catch (err: any) {
      res.status(500).json({ message: err?.message || "Lỗi đọc mật khẩu" });
    }
  });
  ...
  // POST /api/einvoice/sign-schedules — Ký số cho từng đợt thanh toán
  app.post("/api/einvoice/sign-schedules", async (req, res) => {
    if (!(await matbao.isConfigured())) {
      return res.status(500).json({
        message: "Hệ thống chưa cấu hình kết nối Mắt Bão. Vui lòng vào Cấu hình hệ thống → Hoá đơn điện tử.",
      });
    ...
    const parsed = signSchedulesSchema.safeParse(req.body);
    if (!parsed.success) {
      return res.status(400).json({ message: "Dữ liệu gửi không hợp lệ", errors: parsed.error.errors });
    }
    const { scheduleIds, isPublish } = parsed.data;

    const schedRows = await db
      .select()
      .from(invoicePaymentSchedule)
      .where(inArray(invoicePaymentSchedule.id, scheduleIds));

    const invIds = Array.from(new Set(schedRows.map(s => s.invoiceId)));
    const invRows = invIds.length > 0
      ...
      : [];
    const invById = new Map(invRows.map(r => [r.invoice.id, r]));

    const results: Array<{ scheduleId: string; success: boolean; fkey?: string; message: string }> = [];

    for (const sched of schedRows) {
      const r = invById.get(sched.invoiceId);
      if (!r) {
        results.push({ scheduleId: sched.id, success: false, message: "Không tìm thấy hoá đơn gốc" });
        continue;
      }
      const inv = r.invoice;
      const studentName = r.studentName || (inv as any).subjectName || "Khách lẻ";
      const amount = parseFloat(sched.amount ?? "0");
      const lineName = `${inv.category || inv.description || "Học phí"} — ${sched.label}`;
      ...
      try {
        ...
        await db.update(invoicePaymentSchedule)
          ...
          .where(eq(invoicePaymentSchedule.id, sched.id));

        results.push({ scheduleId: sched.id, success: true, fkey: out.fkey, message: out.message });
      } catch (err: any) {
        const msg = err?.message ?? "Lỗi không xác định";
        await db.update(invoicePaymentSchedule)
          .set({
          ...
          .where(eq(invoicePaymentSchedule.id, sched.id));
        results.push({ scheduleId: sched.id, success: false, message: msg });
      }
    }
  ...
  // GET /api/einvoice/schedule-pdf/:scheduleId — Tải PDF của đợt
  app.get("/api/einvoice/schedule-pdf/:scheduleId", async (req, res) => {
    const scheduleId = req.params.scheduleId;
    try {
      const rows = await db.select().from(invoicePaymentSchedule).where(eq(invoicePaymentSchedule.id, scheduleId)).limit(1);
      const sched = rows[0];
      if (!sched) return res.status(404).json({ message: "Không tìm thấy đợt" });
      if (!sched.einvoiceFkey) {
        return res.status(400).json({ message: "Đợt chưa được gửi sang Mắt Bão" });
      }
      const pdf = await matbao.downloadInvoicePdf({
        maSoHDon: sched.einvoiceFkey,
        maTraCuu: sched.einvoiceMaTraCuu,
      });
      const isDraft = sched.einvoiceStatus === "draft";
      const filename = `${isDraft ? "nhap" : "hoadon"}-dot-${sched.code || scheduleId}.pdf`;
