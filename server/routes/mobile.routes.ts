...
export function registerMobileRoutes(app: Express) {
  ...
  /**
   * GET /api/mobile/student/attendance-qr
   *
   * Returns the stable attendance QR token for the logged-in student.
   * Parent accounts receive all linked children's QR tokens, or one child
   * when ?studentId=... is provided.
   *
   * Auth: JWT Bearer token
   */
  app.get("/api/mobile/student/attendance-qr", async (req, res) => {
    try {
      ...
      if (!ctx) return res.status(403).json({ message: "Tài khoản không phải học viên/phụ huynh." });

      const requestedStudentId = typeof req.query.studentId === "string"
        ? req.query.studentId
        : undefined;
      if (requestedStudentId && !ctx.studentIds.includes(requestedStudentId)) {
        return res.status(403).json({ message: "Học viên không thuộc tài khoản này." });
      }

      const targetIds = requestedStudentId
        ? [requestedStudentId]
        : ctx.studentIds;
      const studentMap = new Map(ctx.linkedStudents.map((student) => [student.id, student]));
      const qrStudents = await Promise.all(targetIds.map(async (studentId) => {
        const student = studentMap.get(studentId);
        if (!student) return null;
        const qr = await ensureStudentQrToken(studentId, user.id);
        return {
          student: {
            id: student.id,
            code: student.code,
          ...
          qr: {
            // Encode this value directly in the QR image. Staff scan APIs
            // accept this raw token or the web URL generated by the web app.
            payload: qr.token,
            token: qr.token,
      ...
      return res.json({
        ...
        students: qrStudents.filter(Boolean),
      });
    } catch (err: any) {
      console.error("[Mobile] student/attendance-qr error:", err);
      return res.status(500).json({ message: err.message || "Lỗi khi tải mã QR điểm danh" });
    }
  });
