...
/**
 * Verify a Tinode topic exists AND set its defacs.
 * Returns true  — topic exists and defacs updated OK.
 * Returns false — topic not found (404/403); caller should recreate.
 * On network/timeout error, returns true to avoid spurious recreation.
 */
export async function verifyAndSetTopicDefacs(topicId: string): Promise<boolean> {
  if (!TINODE_URL) return true; // Tinode disabled — don't block
  try {
    const data = await tinodeAdmin.send({
      set: {
        id:    tinodeAdmin.nextMsgId(),
    ...
    const code: number | undefined = data?.ctrl?.code;
    if (code === 404 || code === 403) {
      console.warn(`[Tinode] verifyAndSetTopicDefacs: topic ${topicId} not found (code ${code}) — will recreate`);
      return false;
    }
    return true;
  } catch (err: any) {
    // Network/timeout — assume topic exists to avoid spurious recreation
    console.warn(`[Tinode] verifyAndSetTopicDefacs error for ${topicId}: ${err?.message}`);
    return true;
  }
}
...
/**
 * Xóa hẳn một topic khỏi Tinode (hard delete).
 * Dùng khi nhóm chat bị xóa khỏi DB — đảm bảo không còn ai nhắn tin vào topic cũ.
 * Fire-and-forget an toàn: lỗi chỉ được log, không throw.
 */
export async function deleteTopic(topicId: string): Promise<void> {
  ...
  try {
    ...
    const code = data?.ctrl?.code;
    if (code === 200) {
      console.log(`[Tinode] Deleted topic ${topicId}`);
    } else {
      console.warn(`[Tinode] deleteTopic(${topicId}) returned code=${code}:`, JSON.stringify(data?.ctrl));
    }
  } catch (err: any) {
    console.warn(`[Tinode] deleteTopic(${topicId}) failed (non-critical):`, err?.message);
  }
}
...
export async function removeMemberFromTopic(
  ...
  // Dùng 'del' với what="sub" để kick user khỏi topic.
  // KHÔNG dùng 'leave' vì Tinode 'leave' packet không có field 'user' —
  // nếu dùng 'leave' thì bot sẽ tự unsubscribe khỏi topic thay vì kick user.
  const data = await tinodeAdmin.send({
    del: {
      id:    tinodeAdmin.nextMsgId(),
      topic: topicId,
...
// ─── Cập nhật tên hiển thị user trong Tinode ─────────────────────────────────

/**
 * Sync tên hiển thị (public.fn) của user lên Tinode khi tên thay đổi trong DB.
 * Fire-and-forget an toàn: lỗi chỉ được log, không throw.
 * Yêu cầu bot có quyền admin trong Tinode. Nếu không có quyền (403) thì bỏ qua.
 */
export async function syncUserDisplayName(tinodeUid: string, newName: string): Promise<void> {
  if (!TINODE_URL || !tinodeUid) return;
  try {
    const data = await tinodeAdmin.send({
      acc: {
        id:   tinodeAdmin.nextMsgId(),
        user: tinodeUid,
        desc: { public: { fn: newName } },
      },
    });
    const code = data?.ctrl?.code;
    if (code === 200 || code === 201) {
      console.log(`[Tinode] syncUserDisplayName: uid=${tinodeUid} → "${newName}"`);
    } else {
      console.warn(`[Tinode] syncUserDisplayName(${tinodeUid}) returned code=${code} (non-critical)`);
    }
  } catch (err: any) {
    console.warn(`[Tinode] syncUserDisplayName(${tinodeUid}) failed (non-critical):`, err?.message);
  }
}

/**
 * Wrapper nhận app userId (UUID), tự lookup tinodeUserId trong DB rồi gọi syncUserDisplayName.
 * Dùng từ staff/student update routes — fire-and-forget.
 */
export async function syncUserDisplayNameByUserId(userId: string, newName: string): Promise<void> {
  if (!TINODE_URL || !userId || !newName) return;
  try {
    const [row] = await db.select({ tinodeUserId: users.tinodeUserId }).from(users).where(eq(users.id, userId)).limit(1);
    if (row?.tinodeUserId) await syncUserDisplayName(row.tinodeUserId, newName);
  } catch (err: any) {
    console.warn(`[Tinode] syncUserDisplayNameByUserId(${userId}) failed (non-critical):`, err?.message);
  }
}
 ...
 */
export async function ensureUserInTinode(userId: string, displayName?: string): Promise<{
  tinodeLogin: string;
  ...
  try {
    const data = await tinodeAdmin.send({
      acc: {
        ...
        login:  false,
        desc: {
          public:  { fn: displayName || tinodeLogin },
          private: CENTER_ID
