...
import { normalizeRoleCodePrefix, codeStem, nextCodeForStem } from "../lib/role-code";
...
export async function createRole(role: InsertRole): Promise<Role> {
  const [newRole] = await db.insert(roles).values({
    ...role,
    codePrefix: normalizeRoleCodePrefix(role.codePrefix, role.name),
  }).returning();
  return newRole;
}

export async function updateRole(id: string, updates: Partial<InsertRole>): Promise<Role> {
  const normalizedUpdates = updates.codePrefix === undefined
    ? updates
    : { ...updates, codePrefix: normalizeRoleCodePrefix(updates.codePrefix, updates.name) };
  const [updated] = await db.update(roles).set(normalizedUpdates).where(eq(roles.id, id)).returning();
  return updated;
}
...
export async function createStaff(insertData: any): Promise<Staff> {
  ...
  if (staffData.code && !codeAutoGenerated) {
  ...
  try {
    return await db.transaction(async (tx) => {
      ...
      const roleRows = roleList.length > 0
        ? await tx
            .select({
              id: roles.id,
              name: roles.name,
      ...
      const firstLocationId = locList[0];
      const [firstLocation] = firstLocationId
        ? await tx.select({ code: locations.code }).from(locations).where(eq(locations.id, firstLocationId)).limit(1)
        : [];
      const generatedStem = codeStem(
        firstRole?.codePrefix,
        firstRole?.name,
        firstRole?.codeByLocationRole,
        firstLocation?.code,
      );
      await tx.execute(sql`SELECT pg_advisory_xact_lock(hashtextextended(${generatedStem || "staff"}, 0))`);
      if (codeAutoGenerated || !staffData.code) {
        const [existingStaffCodes, existingUsernames] = await Promise.all([
          tx.select({ code: staff.code }).from(staff).where(sql`UPPER(${staff.code}) LIKE ${`${generatedStem}%`}`),
          tx.select({ code: users.username }).from(users).where(sql`UPPER(${users.username}) LIKE ${`${generatedStem}%`}`),
        ]);
        staffData.code = nextCodeForStem(
          [...existingStaffCodes, ...existingUsernames].map((row) => row.code),
          generatedStem,
        );
      }
