...
export async function createStudent(student: any): Promise<StudentResponse> {
  ...
  return await db.transaction(async (tx) => {
    ...
    const [roleConfig] = await tx
      ...
      .where(and(eq(roles.name, studentData.type), eq(departments.isSystem, true)))
      .limit(1);
    const [firstLocation] = Array.isArray(locationIds) && locationIds[0]
      ? await tx.select({ code: locations.code }).from(locations).where(eq(locations.id, locationIds[0])).limit(1)
      : [];
    const generatedStem = codeStem(
      roleConfig?.codePrefix,
      roleConfig?.name ?? studentData.type,
      roleConfig?.codeByLocationRole,
      firstLocation?.code,
    );
    // Serialize automatic allocation per configured prefix/location.
    await tx.execute(sql`SELECT pg_advisory_xact_lock(hashtextextended(${generatedStem || "student"}, 0))`);

    if (codeAutoGenerated || !studentData.code) {
      const [studentCodes, linkedAccountNames] = await Promise.all([
        tx.select({ code: students.code })
          .from(students)
          .where(sql`UPPER(${students.code}) LIKE ${`${generatedStem}%`}`),
        tx.select({ code: users.username })
          .from(users)
          .where(sql`UPPER(${users.username}) LIKE ${`${generatedStem}%`}`),
      ]);
      studentData.code = nextCodeForStem(
    ...
    const effectiveUsername = String(
      usernameAutoGenerated || !username ? studentData.code : username
    ).trim();
