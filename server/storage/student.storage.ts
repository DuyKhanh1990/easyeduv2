...
export async function createStudent(student: any): Promise<StudentResponse> {
  ...
  return await db.transaction(async (tx) => {
    ...
    // Serialize automatic code allocation per prefix. This lock is held only
    // for the user/student insert portion of the transaction, preventing two
    // simultaneous saves from selecting the same next code.
    ...
    if (codeAutoGenerated || !studentData.code) {
      ...
      studentData.code = `${prefix}${(maxNum + 1).toString().padStart(2, "0")}`;
    }
