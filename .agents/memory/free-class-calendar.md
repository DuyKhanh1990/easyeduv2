...
Lịch tổng `/schedule` phải chiếu mỗi cặp lớp/ngày có đăng ký thành một buổi tổng hợp, kèm danh sách học viên của ngày đó. Điểm danh tại buổi có ba trạng thái: `Chưa điểm danh` (`registered`), `Có học` (`attended`) và `Bảo lưu` (`reserved`).
