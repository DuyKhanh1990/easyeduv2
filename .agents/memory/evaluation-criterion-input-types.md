...
**How to apply:** Dòng không có nhóm khi tạo mới trở thành `heading`; dòng chọn nhóm trở thành `criterion`. Tiêu chí cũ vẫn là `criterion` dạng text để tương thích. API review phải giữ `inputType` và `checked`: checkbox hiển thị ô đã/chưa tick, text hiển thị nội dung nhận xét. Màn hình học viên lấy `groupName` làm tiêu đề nhóm và chỉ hiển thị một `criteriaRating` tổng ở đầu, không lặp sao cho từng dòng con.