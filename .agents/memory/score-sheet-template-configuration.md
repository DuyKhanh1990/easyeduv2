...
**How to apply:** Keep API validation, edit-form state, and conversion switching consistent with these rules. Preserve old linked templates that lack an explicit overall rule. Include assigned assessments in the staff score-sheet timeline by `sessionDate`, with the existing row style and a conversion label. A future results view should use the saved template snapshot and persist its own entered state rather than borrowing legacy grade-book state.