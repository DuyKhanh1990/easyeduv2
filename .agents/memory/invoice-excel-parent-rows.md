...
## Rule
When exporting invoices with multiple payment installments, include the parent invoice row immediately before its visible child installment rows. The parent preserves invoice-level promotion, surcharge, deduction, and aggregate amounts; child rows preserve installment-level collection details.
