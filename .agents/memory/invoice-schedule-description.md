...
Historical cleanup is limited to descriptions matching the known automatic tuition or renewal prefixes with ISO date ranges; manual descriptions must remain untouched.

**Why:** Existing invoice descriptions may be user-edited, so broad date replacement could alter historical notes that are not generated by scheduling.