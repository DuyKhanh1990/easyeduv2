-- Current application insert paths omit created_at in this audited cohort;
-- PostgreSQL fills it with now() using the Asia/Ho_Chi_Minh DB convention.
BEGIN;
...
DECLARE
  ...
  target_tables text[] := ARRAY[
    ...
    'student_locations',
    'shift_templates',
    'teacher_salary_packages',
    'staff_salary_configs',
    'attendance_fee_rules',
    'subjects'
  ];
BEGIN
...
--   backup_table text;
--   target_tables text[] := ARRAY[
--     'crm_pipeline_groups','crm_relationships','crm_reject_reasons',
--     'crm_customer_sources','crm_schools','classrooms','evaluation_criteria',
--     'evaluation_sub_criteria','staff_assignments','student_locations',
--     'shift_templates','teacher_salary_packages','staff_salary_configs',
--     'attendance_fee_rules','subjects'
--   ];
-- BEGIN
--   FOREACH target_table IN ARRAY target_tables LOOP
--     backup_table := target_table || '_created_at_before_utc';
