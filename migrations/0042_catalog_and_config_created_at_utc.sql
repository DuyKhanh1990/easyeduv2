-- These application insert paths omit created_at and rely on DEFAULT now()
-- in the Asia/Ho_Chi_Minh DB session. Preserve the complete old values by ID.
-- The allowlisted table set below was audited for current writers and readers.
BEGIN;
...
DECLARE
  ...
  target_tables text[] := ARRAY[
    ...
    'course_programs',
    'course_program_contents',
    'free_class_session_contents',
    'free_class_day_assignments'
  ];
BEGIN
...
--   backup_name text;
--   target_tables text[] := ARRAY[
--     'locations','departments','roles','courses','course_fee_packages',
--     'score_categories','score_sheets','score_sheet_items','questions',
--     'exam_sections','exam_section_questions','course_programs',
--     'course_program_contents','free_class_session_contents',
--     'free_class_day_assignments'
--   ];
-- BEGIN
--   FOREACH table_name IN ARRAY target_tables LOOP
--     backup_name := table_name || '_created_at_before_utc';
--     EXECUTE format('ALTER TABLE public.%I ALTER COLUMN created_at DROP DEFAULT', table_name);
--     EXECUTE format(
--       'ALTER TABLE public.%I ALTER COLUMN created_at TYPE timestamp without time zone USING created_at AT TIME ZONE %L',
--       table_name, 'Asia/Ho_Chi_Minh'
--     );
--     EXECUTE format(
...
-- $rollback$;
-- COMMIT;
-- Keep all backups until auditing ends.