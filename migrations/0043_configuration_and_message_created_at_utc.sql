-- Application insert paths omit these created_at fields and use PostgreSQL's
-- now()/CURRENT_TIMESTAMP default under the Asia/Ho_Chi_Minh DB convention.
-- Keep all original timestamps for exact rollback.
BEGIN;
...
DECLARE
  ...
  target_tables text[] := ARRAY[
    ...
    'task_levels',
    'zalo_oa_configs',
    'zalo_oa_conversations',
    'zalo_oa_messages',
    'conversations',
    'messages',
    'salary_allowance_types',
    'salary_default_configs'
  ];
BEGIN
  FOREACH target_table IN ARRAY target_tables LOOP
    ...
    END IF;
    IF legacy_default IS NULL OR legacy_default NOT IN ('now()', 'CURRENT_TIMESTAMP') THEN
      RAISE EXCEPTION 'public.%.created_at has an unexpected default: %', target_table, legacy_default;
    END IF;
...
--   backup_table text;
--   target_tables text[] := ARRAY[
--     'ai_settings','invoice_print_templates','payment_gateways',
--     'task_statuses','task_levels','zalo_oa_configs',
--     'zalo_oa_conversations','zalo_oa_messages','conversations','messages',
--     'salary_allowance_types','salary_default_configs'
--   ];
-- BEGIN
--   FOREACH target_table IN ARRAY target_tables LOOP
--     backup_table := target_table || '_created_at_before_utc';
