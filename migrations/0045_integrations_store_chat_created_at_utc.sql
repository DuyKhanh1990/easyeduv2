-- Application writers in this cohort omit created_at and rely on the DB
-- now() default under the Asia/Ho_Chi_Minh session convention.
BEGIN;
...
DECLARE
  ...
  target_tables text[] := ARRAY[
    'admin_hub_connections',
    'omicall_location_configs',
    'store_warehouses',
    'store_suppliers',
    'store_categories',
    'store_units',
...
--   backup_table text;
--   target_tables text[] := ARRAY[
--     'admin_hub_connections','omicall_location_configs','store_warehouses',
--     'store_suppliers','store_categories','store_units','store_colors',
--     'store_sizes','store_products','chat_groups','online_learning_rules',
--     'facebook_page_configs','facebook_conversations','facebook_messages'
--   ];
-- BEGIN
--   FOREACH target_table IN ARRAY target_tables LOOP
--     backup_table := target_table || '_created_at_before_utc';
